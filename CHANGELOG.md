...
### Added
- **Migration 3/7 — les options de taille passent sur l'écran v2, avec une distinction qui manquait (2026-09-25).** `product-size-ui` vise le nouvel écran. Le portage a imposé quatre corrections : (1) **l'ordre des tailles** vient désormais du service partagé `productAttributes` — 40,5 < 42 < 43 pour une pointure, S < M < XL pour un vêtement — au lieu de l'ordre brut du marchand ; (2) **les mots suivent le produit** : une chaussure n'a pas de taille M, un sérum n'a pas de pointure (« Pointure » / « Contenance » / « Taille », et la contenance lue sur la fiche s'affiche même sans variante) ; (3) **l'ajout reste fermé** tant qu'une taille proposée n'est pas choisie — un bouton actif qui se contente d'ouvrir un tiroir fait croire que la commande est partie ; (4) surtout, la boutique distingue enfin **« la source ne confirme pas cette taille »** de **« la source ne publie aucune disponibilité »** (`availabilityKnown`) : sans cette distinction, un simple catalogue de tailles sans données de stock rendait le produit **entièrement incommandable**. Le garde-fou serveur `POST /api/cart` reste l'autorité dans les deux cas, et `unknown` n'est toujours jamais promu en `available` quand la source, elle, suit ses stocks.

- **Migration des garanties vers l'écran v2 — 2 fichiers de tests sur 7 (2026-09-25).** `editorial-product-integrity` et `ayrovix-gallery` visent désormais le nouvel écran. La migration a révélé et corrigé **trois écarts réels** : (1) l'écran affichait le prix marchand brut **avant** le devis serveur — or un prix brut ignore droits, TVA et frais ; il n'affiche plus aucun montant tant que le devis n'est pas là, dit « prix à confirmer », et **ferme l'ajout au panier** (découvrir le prix après le clic serait une promesse trahie) ; (2) la galerie n'avait plus de flèches — sur un écran sans tactile, elle était devenue inaccessible ; (3) le carrousel téléchargeait ses **quatre** photos d'un coup : il ne charge plus que la photo visible et sa voisine, le reste attend. Plafond de quatre photos conservé.

- **Boutique v2 — les dernières commandes de l'ancienne fiche sont portées (2026-09-25).** L'agrandissement plein écran de la photo et les champs repliés de l'équipe d'achat (lien marchand, note) existent de nouveau sur l'écran v2. Deux défauts ont été attrapés **avant** la poussée, pas après : (1) le lien partait sous le nom `sourceUrl` alors que le contrat de commande attend `manualUrl` — il aurait été silencieusement perdu ; (2) ce champ sert d'**URL de la ligne de panier**, donc un champ vidé par le client aurait produit une ligne sans adresse : il est pré-rempli avec le lien marchand connu et, à l'envoi, retombe dessus s'il est vide. 4 tests supplémentaires.

- **Ligne descriptive des résultats Lens — obtenue, pas fabriquée (2026-09-25).** Google Lens rend un titre, une boutique et un prix, mais **aucune description** : la carte affichait donc une ligne grise vide, et fabriquer ce texte à partir du titre restait interdit. La seule voie honnête était de le **demander** — un appel `google_shopping` par produit. La dépense est encadrée : **budget** de 4 résultats (les seuls que l'œil lit), **cache disque de 7 jours** par titre normalisé (une description ne bouge pas comme un prix), **concurrence 2**, **échéance stricte** (l'enrichissement ne retarde jamais la réponse ; s'il n'est pas prêt, on répond sans lui), et `AYROVI_LENS_ENRICH=false` coupe la dépense sans redéploiement. Surtout : **la preuve avant le texte** — une description n'est retenue que si la fiche trouvée recouvre fortement le titre du produit (60 % des mots signifiants) ; décrire un produit avec le texte d'un autre serait pire que se taire. Un échec réseau n'est pas mémorisé (on réessaie), une absence de résultat l'est (on ne repaie pas pour du vide). 10 tests dans `tests/lens-enrichment.test.ts`.

- **BOUTIQUE v2 INSTALLÉE — la fiche produit de la maquette est désormais celle que voit le client (2026-09-25).** Les trois surfaces qui affichaient l'ancienne fiche (Lens, tiroir produit, assistant) rendent maintenant `ShopProductScreen`. Le conteneur expose **exactement la même interface** que l'ancienne fiche : la bascule n'a demandé aucun autre changement sur les sites d'appel, et un retour arrière tiendrait en une ligne. Tout ce qui parle au serveur vit dans le conteneur (devis de prix, couleur active, ajout au panier) ; l'écran reste muet, donc testable sans réseau. Deux commandes de l'ancienne fiche ont été **rétablies avant la bascule** plutôt que perdues en silence : la **quantité** (sans elle, il aurait fallu commander une pièce à la fois) et **« Calculer un autre article »**. Le prix affiché vient toujours du serveur (`/api/public/pricing/cart-line`) : droits, TVA et taux changent sans redéploiement du client.

  **Restent à porter avant de supprimer l'ancienne fiche** (guarantees non encore migrées, donc rien n'est supprimé) : l'agrandissement plein écran de la photo, et le formulaire avancé de l'équipe d'achat (lien manuel, note client). L'ancien composant reste en place tant que ces deux points ne sont pas couverts par des tests sur le nouvel écran — supprimer sans migrer la garantie, c'est perdre la garantie.

- **Moteur Lens — une échéance couvre enfin le CUMUL des tentatives (2026-09-25).** Chaque moteur avait déjà sa coupure réseau (vision 8 s, recherche visuelle 10 s), mais la vision **réessaie une fois** en cas de réponse malformée : deux tentatives plus la latence pouvaient dépasser vingt secondes, pendant lesquelles le client regardait un écran muet. Aucun plafond ne couvrait ce cumul. `recognizeImage` applique désormais une échéance par source (vision 18 s, offres 14 s, bornée entre 2 et 30 s, réglable par `AYROVI_LENS_*_DEADLINE_MS`) : à son expiration on **rend ce qui est prêt** — souvent les offres marchandes, qui suffisent à afficher un résultat. Un résultat arrivé trop tard n'est **ni affiché ni mis en cache** : il correspondrait à une requête que le client a déjà quittée. Les étapes abandonnées sont journalisées et exposées dans `timedOut`, jamais silencieuses. 3 tests, dont deux comportementaux (un moteur qui ne répond jamais n'immobilise plus la requête ; une réponse tardive ne pollue pas le cache).
...
- **Moteur Lens — nous cessons de jeter ce que la source affirme (2026-09-25).** Google Lens renvoie `in_stock` et `condition` sur chaque correspondance : les deux étaient **ignorés**, et l'état du produit était ensuite **deviné** à partir de mots trouvés dans le titre. Conséquence : un titre malheureux (« sneakers vintage condition look ») faisait disparaître un produit neuf, et une occasion au titre soigné passait. Désormais la source parle en premier — l'état déclaré tranche, la lecture par mots-clés ne sert plus que lorsqu'elle se tait — et `in_stock` alimente la disponibilité du candidat, **un silence restant `unknown`**, jamais promu en disponibilité. Zéro appel supplémentaire : ces faits arrivaient déjà dans la réponse. 7 tests dans `tests/lens-source-facts.test.ts`.

- **Moteur Lens — UNE seule orchestration, enfin (2026-09-25).** Le défaut de fond relevé par l'audit est corrigé : il existait deux séquences pour la même question (« que voit-on sur cette image ? ») — celle de `runLensPipeline` et une copie écrite à la main dans la route publique, qui avaient déjà divergé. `lensEngine.recognizeImage()` est désormais le seul endroit qui sait dans quel ordre interroger la vision, la recherche visuelle et les signaux de l'image ; la route publique et le pipeline interne l'appellent tous les deux et ne gardent que ce qui leur appartient (format de réponse d'un côté, cache en base et fusion de l'autre). Les imports devenus faux ont été retirés du pipeline : un fichier ne doit pas avoir l'air de savoir faire ce qu'il ne fait plus. Garanties tenues par 9 tests (`tests/lens-engine-unified.test.ts`), dont trois comportementaux : la vision peut tomber pendant que la recherche visuelle réussit, la seconde analyse d'une même image ne rappelle aucun moteur payant, et **un échec n'est jamais mémorisé** — la requête suivante réessaie vraiment.
- **Le cache Lens est inactif sous test, sauf répertoire isolé.** Défaut découvert pendant l'intégration, pas en production : un cache sur disque rendait les exécutions dépendantes les unes des autres — un scénario de l'assistant passait « vert » parce qu'une entrée écrite par un test précédent servait la réponse, et les moteurs n'étaient jamais appelés. Un test doit décrire le système, pas l'historique de la machine.

- **Moteur Lens — le chemin CLIENT lit enfin l'image (OCR + code-barres), 2026-09-25.** Second écart relevé par l'audit : `runLensPipeline` (chemin interne) lisait le texte de la photo en plusieurs passes et cherchait un code-barres, pendant que `POST /analyze-image` ne faisait ni l'un ni l'autre — même photo, reconnaissance plus pauvre pour le client que pour l'administrateur. `lensSignals` apporte ces lectures au chemin client **sans réécrire une seule ligne de logique** (il appelle `prepareImageForAnalysis`, `ocrRecognize`, `analyzeOcrText`, `scanCodeFromImage`). Elles tournent **en parallèle** des deux moteurs — calcul local, aucun appel payant — sous **budget de temps** (`AYROVI_LENS_SIGNALS_BUDGET_MS`, 4,5 s) : au-delà, on rend ce qui est prêt, car mieux vaut une reconnaissance sans OCR qu'un client qui attend. Arbitrage du prix lu sur l'image : **la vision garde la main tant qu'elle est sûre**, l'OCR ne parle que lorsqu'elle doute et seulement s'il est sûr lui-même, et si les deux doutent le prix reste *non lu* — on ne devine jamais un montant que le client finirait par payer. Un nombre sans devise n'est pas un prix ; un total de panier est étiqueté comme tel. Ces signaux décrivent la photo : ils sont donc mis en cache avec elle (24 h), jamais avec le marché. Nouvelle mesure `imageSignalsMs` déclarée dans le contrat de trace. 11 tests dans `tests/lens-signals.test.ts`.

- **Moteur Lens — le chemin CLIENT a enfin un cache, et une mesure qui ne ment plus (2026-09-25).** L'audit a montré que `POST /analyze-image` n'avait aucun cache alors que le chemin interne en a un : la même photo envoyée deux fois payait deux fois la vision ET deux fois SerpApi, et faisait attendre deux fois. `lensRecognitionCache` mémorise désormais séparément ce qui décrit la **photo** (marque, modèle, prix lu — 24 h, car une photo ne change pas) et ce qui décrit le **marché** (correspondances marchandes et leurs prix — 30 min, car au-delà un prix affiché n'est plus une observation mais un souvenir). **Le calcul de prix n'est jamais mis en cache** : droits, TVA et taux sont refaits à chaque requête depuis les règles en base. Une liste vide n'est pas mémorisée (un incident passager ne se sert pas pendant trente minutes), une entrée corrompue vaut absence, et `AYROVI_LENS_CACHE=false` coupe tout sans redéploiement. La télémétrie mesure enfin **chaque moteur séparément** — l'ancienne version partait du même instant pour les deux appels parallèles et enregistrait donc deux fois la même durée, ce qui rendait impossible de savoir lequel ralentissait la recherche. En-tête `X-Ayrovix-Cache` sur la réponse. 10 tests dans `tests/lens-recognition-cache.test.ts`.

- **BOUTIQUE v2 — module neuf, autonome, sans une ligne d'ancien code (2026-09-25).** `client/src/shop/` : `ProductGrid`, `ProductPage`, `SizeDrape`, `BagPage`, `AddressPage`, `PaymentPage`, `LoadingView`, plus le contrat `ProductView` et son unique adaptateur. Trois principes tenus par des tests : (1) **un seul contrat de données** — les écrans n'ont plus le droit de calculer un prix, de convertir une taille ou de fabriquer un texte, ils affichent une vue déjà juste ; (2) **`null` s'affiche comme une absence**, jamais comme une valeur par défaut (pas de prix à 0,00, pas de marque devinée, pas de quantité inventée — aucune source publique ne l'expose) ; (3) **indépendance vérifiable** — aucun écran v2 n'importe un composant ancien, et `shop.css` n'utilise que ses propres classes `s-`, sans un seul orange, en lisant les jetons officiels de `identity.json`. La fiche reproduit la maquette à la lettre : produit entier au repos, feuille qui monte et éteint l'image, actions posées sur la photo, barre d'achat qui ne quitte jamais l'écran, drap des tailles à deux échelles rendant les **trois** états du moteur — et `unknown` reste non commandable, au panier comme à la fiche. 16 tests dans `tests/shop-v2.test.tsx` ; plan d'exécution dans `PLAN-BOUTIQUE-V2.md`.
- **Repli d'image sur NOTRE canvas quand la composition décline (2026-09-25).** Défaut vu en production : dans une grille de quatre cartes, trois sur notre gris studio et une avec un rectangle blanc — la photo du marchand renvoyée telle quelle. Décliner le détourage ne dispense pas de tenir le cadre : l'image est désormais posée entière (contain, sans rognage) sur le canvas 9/13 `#F0F2F2`, en-tête `X-Ayrovi-Card: letterbox` à l'appui. La redirection brute ne reste que le tout dernier recours.

### Fixed
- **Le panier flottant de la fiche recouvrait la flèche « photo suivante »** (deux cibles tactiles superposées, une action perdue) : le rail remonte au-dessus des contrôles de galerie.

### Added
- **Panier au format de la maquette (2026-09-25).** Vignette 88 × 116 en 3/4 servie par le cadre studio — donc **la même composition AYROVI** que la carte et la fiche, plus d'image marchand dépareillée d'un écran à l'autre. Le tiroir **glisse** désormais depuis le bord dont il dépend (droite en LTR, **gauche en RTL** : en arabe il arrivait du côté opposé à son bouton) et le voile se fond, 240/180 ms. Sous la CTA, un bandeau des **moyens de paiement réellement disponibles**, lu depuis la source unique `availablePaymentMethods(policy)` : le panier ne recopie aucune règle et n'annonce jamais une marque que la caisse refusera. 7 tests dans `tests/cart-drawer-shape.test.ts`.

- **Drap des tailles : deux échelles, et seulement quand elles existent (2026-09-25).** Onglets « Taille EU » / « Taille marque » construits à partir des libellés RÉELLEMENT reçus de la source (une taille n'entre dans l'échelle marque que si son libellé est unique et différent de la taille) — **aucune table de conversion maison**, on ne devine pas la pointure du client à sa place. Les onglets disparaissent quand la source ne donne qu'une échelle. Le drap **monte** désormais (240 ms) au lieu d'apparaître : un panneau qui apparaît se lit comme un changement de page, un panneau qui monte se lit comme un tiroir qu'on peut refermer. Les trois états du moteur de disponibilité restent intacts.
- **« On vérifie » n'est plus affiché comme « à confirmer » (2026-09-25).** Pendant l'appel de tarification, la fiche annonçait *Prix à confirmer* — un échec, alors que rien n'a échoué. L'attente a maintenant sa propre forme (rotor sobre + « Vérification du prix à la source… ») ; le message d'échec, lui, reste réservé aux vrais échecs. Le drapeau retombe dans un `finally` : jamais de rotor éternel.

- **La grande fiche affiche enfin la MÊME image que la petite carte (2026-09-25).** Défaut signalé par le client : la carte montrait le produit détouré posé sur notre fond studio (composition phase 2), pendant que la fiche s'arrêtait à l'isolation seule (phase 1) — même produit, deux rendus. `productMediaChain()` devient la chaîne unique des deux surfaces : **composition → isolation → image marchand**, avec recul d'un cran sur `onError` (jamais de trou).
- **Fiche produit au format téléphone (2026-09-25).** Le panier quitte l'en-tête et se pose **sur la photo** en pastille noire, là où le pouce l'atteint ; la CTA devient une **barre d'achat fixe** en bas, et la page rend l'espace correspondant pour que rien ne finisse caché dessous. Le bouton secondaire reste dans le flux : une barre fixe à deux boutons mange l'écran et brouille l'action principale. Le format est détecté par `matchMedia` et **le balisage n'est jamais dupliqué** — un seul exemplaire de chaque bouton, donc un seul nom accessible.

- **Fiche produit « feuille montante » (mobile), modèle validé sur maquette le 2026-09-25.** Au repos le produit est **entier**, plein cadre, sur notre fond studio, et les photos défilent latéralement ; dès que la page remonte, le panneau d'information glisse **par-dessus** l'image et celle-ci **s'éteint derrière lui** (voile d'encre pure plafonné à 58 %, recul de 4 % à échelle unique X=Y — aucune déformation). Tout est piloté par **une seule variable**, `--ay-pdp-reveal`, écrite par `ProductResult` au défilement (rAF, passif) et lue par la nouvelle feuille `client/src/styles/product-sheet.css` ; les commandes flottantes de la photo s'effacent et se désactivent quand la feuille les recouvre, plutôt que de rester à moitié cliquables. Strictement mobile (`max-width: 767px`) : la mise en page bureau en deux colonnes est inchangée. `prefers-reduced-motion` respecté. 7 tests de contrat dans `tests/product-sheet.test.ts`.
- **Carte de résultat — le descriptif se lit comme la ligne de prix barrée (2026-09-25).** Sous le titre, le descriptif prend EXACTEMENT la taille, le gris et le poids du prix barré de la grille (0,75 rem / 400 / `#595959`), sur **une seule ligne** : deux lignes désalignaient la grille et poussaient le prix hors du cadre. Parité vérifiée par test contre `.ay-quiet-price--grid .ay-quiet-price__original`.

- **Moyens de paiement tunisiens — le paysage complet, sans promesse creuse (2026-09-25).** La caisse affiche désormais **huit** moyens : carte bancaire (réseaux **Visa** et **Mastercard** montrés comme marques, jamais comme options — on ne « choisit » pas un réseau, il suit la carte), virement bancaire, transfert postal, **Flouci**, **D17**, **Ooredoo Money**, **Orange Money** et **cartes Sodexo**. La règle de disponibilité reste la source unique `client/src/commerce/paymentMethods.ts` : un moyen sans passerelle réellement configurée s'affiche **désactivé avec sa raison factuelle** (« en attente d'une passerelle réelle », « acceptation marchande non configurée ») — aucune transaction n'est jamais simulée, et le pied de page continue de ne lister que ce qui encaisse vraiment. Logos officiels ajoutés dans `client/public/media/payments/` (visa, mastercard, ooredoo, orange, sodexo) ; **D17 est rendu en mot-marque faute de fichier officiel — aucun logo n'est fabriqué**. 5 tests dans `tests/payment-methods-tunisia.test.ts`, dont la vérification que chaque marque image existe réellement dans le dépôt.

- **Maquette de référence des écrans boutique** (`mockup-shop-screens/MOCKUP.html`, hors dépôt) : carte, fiche, tailles, chargement, panier, paiement, adresse — générée depuis `glyphs.json` / `identity.json` et les polices officielles, **sans aucun orange** (l'action est noire, conformément à l'identité).

### Added
- **Moteur de VARIANTES & DISPONIBILITÉ (phase 3 du prototype validé le 2026-09-25).** `src/ayrovix/services/productVariants.ts` comprend une réponse marchande sans rien deviner : il lit les **quatre formes** de variantes publiées par la source (`product_results.variations` objet-de-listes, `product_results.variants` liste-de-groupes, `product_results.sizes`, `product_variations`), puis déduit la **nature d'un attribut de SES VALEURS, jamais du nom choisi par le marchand** — un groupe appelé « Size » dont les valeurs sont « 50 ml / 100 ml » est un **Volume**, et des pointures « 39, 40, 41 » proposées sur un parfum sont **écartées avec leur raison** (matrice de compatibilité catégorie × nature : chaussure, haut, bas, parfum, téléphone…). Chaque variante conserve son **chemin JSON d'origine** : rien n'est inventé, rien n'est complété par une liste supposée. La disponibilité a **trois états** — `available` (drapeau ou texte explicite de la source), `unavailable` (rupture constatée, reconnue aussi en français et en arabe), `unknown` — et **`unknown` n'est jamais promu en `available`**, à aucune étape. `src/ayrovix/services/variantAvailability.ts` comble le trou des formes qui ne transportent aucun stock (`product_results.sizes` notamment) par un **appel dédié à la source pour la variante elle-même** (son propre `product_id`/`serpapi_link`), avec cache disque (`AYROVI_VARIANT_CACHE_DIR`, TTL 6 h), budget par produit (`AYROVI_VARIANT_MAX_LOOKUPS`, 8), concurrence bornée et coupe-circuit (`AYROVI_VARIANT_LOOKUP=false`) : un appel en échec, hors budget ou sans clé laisse `unknown` — il ne suppose rien. `AyrovixVariantOption` gagne le champ tri-état `availability` (le booléen `available`, qui signifie « choix possible » et non « en stock », reste inchangé pour compatibilité).
- **Porte de commande côté serveur (`guardVariantOrder`).** La disponibilité envoyée par le navigateur n'est pas une preuve : `POST /api/cart` relit le **contrat que le serveur a lui-même établi** pour ce produit et refuse en `409` toute variante en rupture (`VARIANT_UNAVAILABLE`), au stock non confirmé (`VARIANT_AVAILABILITY_UNKNOWN`) ou absente des données du marchand (`VARIANT_NOT_FOUND`). **Non-régression assumée :** sans contrat enregistré, le parcours historique (demande manuelle au prix général) passe exactement comme avant. Côté client, la carte produit affiche l'état réel de chaque taille — « Indisponible » ou « Stock non confirmé » — et explique pourquoi la commande est bloquée au lieu de griser un bouton sans raison.
- Vérification exécutable : `npm run verify:product-variants` (7 réponses réelles, dont le piège du groupe mal nommé, le produit sans variante et les ruptures écrites en clair — **7/7 conformes via le code de production**) et 17 tests unitaires dans `tests/product-variants.test.ts`.

### Added
- **Moteur de COMPOSITION d'image produit (phase 2 du prototype validé le 2026-09-25).** `src/services/imageComposition.ts` pose le produit détouré sur le mockup officiel (cadre 9/13, canvas `#F0F2F2`, rayon `18/760` — miroir exact de `lens-product-card.css`) : fond et produit restent DEUX COUCHES jusqu'au composite final. Les règles sont générales et exprimées en ratios — **un seul facteur d'échelle pour X et Y** (ratio conservé par construction, écart mesuré ≤ 0,07 %), **contain-fit** dans une zone de sécurité 8,5 % / 7 % / 9 % (le rognage devient structurellement impossible), centrage sur le **barycentre alpha** (et non la boîte englobante) vers un centre optique à 48 %, et une **résolution de rendu bornée par la qualité** : une vignette 183×275 obtient le même équilibre visuel sans être interpolée au-delà de ×3,2. Aucune dimension codée en dur, aucune règle par image. Nouvelle route `GET /api/public/media/card?u=…&w=600|760|900|1080` (cache disque par URL+largeur, `AYROVI_COMPOSED_CACHE_DIR`) : le PNG composé n'est servi QUE si le contrat d'acceptation est tenu, sinon redirection vers l'original — jamais d'image cassée. Côté vitrine, `composedMediaUrl()` passe en tête de la chaîne de repli et `.lens-card-media>img[data-composed]` bascule en `object-fit: contain` sans `mix-blend-mode`. Vérification exécutable : `npm run verify:image-composition` (6 images réelles, 6/6 conformes) et 29 tests unitaires dans `tests/image-composition.test.ts`.

### Fixed
- **Le masque de segmentation était lu au tiers de sa largeur — c'est l'origine des produits « striés, décalés, déformés ».** `sharp` promeut une entrée RAW 1 canal en sortie sRGB **3 canaux** : `segmentBuffer` indexait le tampon rendu comme du 1 canal (pixel `i` au lieu de `3i`). Le masque est désormais ramené explicitement à un canal (`toColourspace('b-w')` + `firstChannel()`).
- **Le masque ne coïncidait pas avec le produit.** L'image était envoyée au modèle en « contain » (donc entourée d'un padding dans le carré 320×320) mais le masque rendu était étiré en « fill » vers la taille native, padding compris — bords rognés d'un côté, fond conservé de l'autre. `containBox()` extrait maintenant la zone utile du carré avant la remise à l'échelle.
- **Les vraies ouvertures étaient rebouchées** (anse de sac, poignée, espace entre deux pièces d'un lot) : `fillMaskHoles()` ne comble que les trous < 0,8 % de l'aire du sujet, les grandes ouvertures restent transparentes.
- **Liseré du studio marchand autour du produit** : `decontaminateFringe()` retrouve la couleur pure des pixels semi-transparents (`P = (C − (1−a)·F) / a`) — les pixels pleinement opaques ne sont jamais touchés.
- Pour la composition, un fond **blanc** est désormais détouré lui aussi (`isolateBuffer(buffer, { treatWhiteAsUniform: true })`) : le `mix-blend-mode: multiply` qui masquait le blanc n'existe plus sur une carte composée.

### Added
- **Admin — « Console opérationnelle » (modèle A, Amazon Seller Central), retenu et appliqué le 2026-09-22.** L'adoption passe par le **socle** (coquille + primitives partagées), pas par une réécriture des écrans : les 49 écrans du back office héritent du modèle sans qu'aucun contrat ne bouge. `client/src/styles/admin-console.css` (nouvelle feuille additive, chargée après `admin.css`) porte les rôles du modèle et les reporte sur les variables `--admin-*` ; `BackOfficeShell.tsx` est recomposé en deux barres (encre + services = les groupes de la navigation serveur), un rail qui n'affiche que le service actif, un fil d'Ariane mince et un filtre de domaine contextuel. Retirés parce qu'en trop : le bloc « Au quotidien » (quatre écrans recopiés — « Tableau de bord » et « Commandes » apparaissaient deux fois), les pastilles « legacy » / « doublon » (l'information passe en infobulle, fil d'Ariane et bandeau de l'écran), le filtre de domaine là où le service ne mélange rien, et le titre d'écran affiché deux fois. Densité compacte par défaut. Palette inchangée : encre `#111110`, orange d'action `#FF6900`. Détail complet et captures : `docs/admin-model-a/README.md`.

### Fixed
- **Le menu de l'admin était inatteignable sur téléphone** : `@media (max-width: 640px) .admin-icon-button { display: none }` masquait aussi le bouton d'ouverture du rail, qui n'avait alors aucun autre point d'entrée. Le bouton a ses propres règles dans la couche du modèle A et le tiroir suit le point de rupture historique (900 px).
- **Barre publique — les libellés arabes étaient la copie du français.** Un visiteur qui lisait le site en arabe voyait « Arrivage », « Gift & Cards », « Magazine » en latin alors que le reste de la page était traduit. Le contrat partagé porte maintenant `وصلات جديدة` · `هدايا وبطاقات` · `مجلة AYROVI` (source unique, reprise par l'Admin, l'API, le routage et les pages plein écran), et une réparation ponctuelle (`public_nav_arabic_labels_v1`, via `runOnceDataMigration`) réaligne les lignes dont le libellé AR est resté identique au FR — un libellé retouché à la main par l'Admin n'est jamais écrasé.
- `PublicCmsSections.tsx` recopiait les trois libellés de la barre publique : il lit désormais le contrat partagé (`client/src/navigation/publicPages.ts`) comme le reste de l'application ; seuls « Social » et les textes éditoriaux restent locaux.
- Écrans du moteur de ressources : « Journal du premier résultat », qui ouvrait l'historique d'une ligne choisie au hasard, est remplacé par une action **Journal** par ligne et par le compte de résultats dans la barre d'outils ; la légende du tableau n'est plus un bandeau gris.
- Libellés d'écrans incohérents ou porteurs de dette technique : `Hero Management` → **Visuels d'accueil**, `AI Discovery` → **Découverte IA**, `Lens Test Lab` → **Lens — banc d'essai**, `Produits (ancienne surface)` → **Produits**, `Marques (ancienne surface)` → **Marques**, `Journal d'audit (legacy)` → **Journal d'audit**.
- Code mort supprimé : `client/src/components/NavigationBrandIcons.tsx` (ré-export déprécié sans import) et `client/src/components/StartShoppingGates.tsx` (jamais monté). `DiscoveryHub.tsx`, `HeroSlider.tsx` et `assistant/AssistantVoiceOrb.tsx` restent en place : ils sont conservés sur décision documentée, et leur suppression est proposée dans `docs/admin-model-a/README.md`.
