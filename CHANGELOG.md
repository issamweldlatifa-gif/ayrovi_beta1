...
- Accepted and implemented the realtime architecture in `docs/REALTIME_VOICE_ARCHITECTURE_AR_2026-08-29.md`: dedicated `gemini-3.5-transcribe-live` → Claude agent/tools → deterministic streaming TTS, without creating a second Gemini assistant.
- Added direct Gemini Live Transcribe over constrained single-use ephemeral tokens and AudioWorklet PCM16/16 kHz; the long-lived Gemini key remains server-only.
- Kept the complete MediaRecorder container beside Live strictly as automatic Groq recovery, while one finalized transcript remains the only gateway into Claude.
- Added one server SSE-to-PCM TTS proxy and one cancellable Web Audio scheduler for `gemini-3.1-flash-tts-preview`; production stays browser-only until quota is proven.
...
- Live token 429 has an independent bounded circuit; token/session failures silently fail closed to Groq instead of breaking Voice.
- Live sessions renew before the ten-minute limit or on `goAway`, only between turns; PCM backpressure is dropped rather than queued without bounds.
- Streaming TTS never launches a duplicate browser utterance after PCM playback has begun, and cancellation stops every scheduled source in the one output operation.
...
- Added regressions for ephemeral-token constraints/key isolation, Live interim/final/backpressure/renewal/fallback, duplicate-turn suppression, ordered PCM streaming, full-operation cancellation, and streaming 429 call suppression.
- Added a regression proving both legacy and streaming TTS requests stop provider calls after the first Gemini 429.
- Full validation passes: 240/240 automated tests, server/client TypeScript checks, and the production build.
