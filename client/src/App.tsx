    ...
    getCommerceConfig()
      .then((payload) => {
        ...
        if (theme && typeof theme === 'object' && theme.primary) {
          // Seules les substitutions sémantiques pilotées par l'Admin sont
          // appliquées. Aucune variable décorative ou héritée n'est touchée.
          const primary     = String(theme.primary);
          const hero        = String(theme.hero || primary);
          const canvas      = String(theme.surface || '#ffffff');
          const canvasAlt   = String(theme.surfaceAlt || '#f7f7f8');
          const textPrimary = String(theme.ink || primary);
          const danger      = String(theme.danger || '#a63b32');
          const success     = String(theme.success || '#2f6b4f');
          const info        = String(theme.info || '#356a8c');
          const warning     = String(theme.warning || '#8a5a14');

          const set = (name: string, value: string) => root.style.setProperty(name, value);
          set('--color-ink', primary);
          set('--color-hero-bg', hero);
          set('--color-canvas', canvas);
          set('--color-canvas-alt', canvasAlt);
          set('--color-surface-base', canvas);
          set('--color-surface-alt', canvasAlt);
          set('--color-text-primary', textPrimary);
          set('--color-interactive-primary', primary);
          set('--color-brand', primary);
          set('--color-success', success);
          set('--color-info', info);
          set('--color-warning', warning);
          set('--color-danger', danger);

          set('--ayrovi-primary', primary);
          set('--ayrovi-primary-dark', primary);
          set('--ayrovi-hero-bg', hero);
          set('--ayrovi-canvas', canvas);
          set('--ayrovi-canvas-alt', canvasAlt);
          set('--ayrovi-surface-base', canvas);
          set('--ayrovi-surface-alt', canvasAlt);
          set('--ayrovi-neutral-50', canvas);
          set('--ayrovi-neutral-950', hero);
          set('--ayrovi-neutral-900', textPrimary);
          set('--ayrovi-text-primary', textPrimary);
          set('--ayrovi-ink', textPrimary);
          set('--ayrovi-interactive-primary', primary);
          set('--ayrovi-accent', primary);
          set('--ayrovi-deep', hero);
          set('--ayrovi-gradient', `linear-gradient(180deg, ${hero} 0%, ${primary} 100%)`);
          set('--ayrovi-success', success);
          set('--ayrovi-info', info);
          set('--ayrovi-warning', warning);
          set('--ayrovi-danger', danger);
          set('--ayrovi-rhubarb', danger);
        }
