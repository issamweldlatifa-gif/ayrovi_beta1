    ...
    getCommerceConfig()
      .then((payload) => {
        ...
        if (theme && typeof theme === 'object' && theme.primary) {
          ...
          const success = String(theme.success || AYROVI_SEMANTIC_PALETTE.success);
          const info = String(theme.info || AYROVI_SEMANTIC_PALETTE.info);
          const warning = String(theme.warning || AYROVI_SEMANTIC_PALETTE.warning);
          const accentGold = String(theme.accentGold || AYROVI_SEMANTIC_PALETTE.accentGold);
          ...
          root.style.setProperty('--color-accent-gold', accentGold);
          root.style.setProperty('--ayrovi-primary', primary);
          root.style.setProperty('--ayrovi-primary-dark', String(theme.primaryDark || AYROVI_SEMANTIC_PALETTE.heroBackground));
          root.style.setProperty('--ayrovi-primary-light', 'color-mix(in srgb, var(--color-interactive-primary) 10%, var(--color-surface-base))');
          root.style.setProperty('--ayrovi-hero-bg', hero);
          root.style.setProperty('--ayrovi-neutral-950', hero);
          root.style.setProperty('--ayrovi-accent', primary);
          root.style.setProperty('--ayrovi-gold', accentGold);
          root.style.setProperty('--ayrovi-neutral-50', surfaceBase);
          ...
          root.style.setProperty('--ayrovi-success', success);
          root.style.setProperty('--ayrovi-info', info);
          root.style.setProperty('--ayrovi-danger', danger);
          root.style.setProperty('--ayrovi-warning', warning);
