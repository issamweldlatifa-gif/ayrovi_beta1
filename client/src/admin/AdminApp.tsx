...
const navGroups = [
  ...
  { label: 'الواجهة', items: [{ id: 'interface', label: 'واجهتي', icon: Eye, permission: 'content:read' as Permission }] },
  { label: 'Contenu', items: [
    ...
    <Modal open={Boolean(selected)} title={selected?.order_number || 'Commande'} onClose={()=>setSelected(null)} wide>{detailLoading ? <PageLoading/> : selected && <div className="admin-order-detail">
      ...
      {selected.deposit_status && selected.deposit_status!=='NONE' && <section className="admin-list-card" style={{marginBottom:16}}>
        ...
        {selected.deposit_proof_path
          ?<a className="admin-button admin-button--secondary" href={`/api/admin/orders/${selected.id}/deposit-proof`} target="_blank" rel="noreferrer"><Eye size={14} />Voir la preuve d’acompte</a>
...
const DesignPage:React.FC<{canWrite:boolean}>=({canWrite})=>{
  ...
  return <><PageHeader title="Développement & design" description="Palette monochrome AYROVI : blanc Apple, gris neutres et noir Apple pour la structure ; vert succès, bleu info, ambre avertissement et rouge danger réservés aux statuts. Le pied de page et les canaux restent administrables." action={canWrite?<Button busy={busy} onClick={save}>Publier le design</Button>:undefined}/>
  <div className="admin-report-grid">
