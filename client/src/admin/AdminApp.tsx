...
const CustomersPage: React.FC = () => {
  ...
  return <><PageHeader title="Clients" description="Une fiche client unique peut posséder plusieurs commandes et conserve sa valeur historique."/><section className="admin-list-card"><div className="admin-list-toolbar"><Search value={search} onChange={setSearch} placeholder="Nom ou téléphone…"/></div><DataTable rows={rows} loading={loading} onRowClick={open} columns={[
    {key:'name',label:'Client',render:(r)=><div><strong>{r.name}</strong><small className="admin-block-small">{r.phone}</small></div>},{key:'governorate',label:'Gouvernorat'},{key:'order_count',label:'Commandes'},{key:'lifetime_value',label:'Valeur client',render:(r)=>formatMoney(r.lifetime_value)},{key:'status',label:'Statut',render:(r)=><StatusBadge status={r.status}/>},{key:'registered_at',label:'Client depuis',render:(r)=>formatDate(r.registered_at)}]}/><Pagination {...page} onChange={load}/></section>
    <Modal open={Boolean(selected)} title={selected?.name||'Client'} onClose={()=>setSelected(null)} wide>{selected&&<><div className="admin-order-summary"><article><span>Téléphone</span><strong>{selected.phone}</strong></article><article><span>Gouvernorat</span><strong>{selected.governorate}</strong></article><article><span>Adresse</span><strong>{selected.address}</strong></article></div><h3>Commandes du client</h3><DataTable<any> rows={selected.orders||[]} columns={[{key:'order_number',label:'Commande'},{key:'status',label:'Statut',render:(r)=><StatusBadge status={r.status}/>},{key:'total_tnd',label:'Total',render:(r)=>formatMoney(r.total_tnd)},{key:'created_at',label:'Date',render:(r)=>formatDate(r.created_at,true)}]}/></>}</Modal></>;
};
...
const AdminShell:React.FC<{user:UserIdentity;onLogout:()=>void}>=({user,onLogout})=>{
  ...
  let page:React.ReactNode;if(section==='dashboard')page=<DashboardPage/>;else if(resources[section])page=<ContentPage resource={section} canWrite={has(resources[section].permission)}/>;else if(section==='orders')page=<OrdersPage canWrite={has('orders:write')} canPay={has('payments:write')}/>;else if(section==='customers')page=<CustomersPage/>;else if(section==='pricing')page=<PricingPage canWrite={has('pricing:write')}/>;else if(section==='settings')page=<SettingsPage canWrite={has('settings:write')}/>;else if(section==='users')page=<UsersPage/>;else if(section==='audit')page=<AuditPage/>;
