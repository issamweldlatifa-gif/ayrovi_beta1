...
import {
  AlertCircle, Bell, Calculator, Calendar, Camera, ChartLine, CheckCircle2, CreditCard, Eye, FileText, FigLeaf, Gift, Globe2, Grid,
  History, Home, Image, Link2, LogOut, Menu, MessageSquare, Package, Palette, Pencil, Percent, Plus, Search as SearchIcon,
...
const LoginPage: React.FC<{ onAuthenticated: (user: UserIdentity) => void }> = ({ onAuthenticated }) => {
  ...
  return (
    <main className="admin-login">
      ...
      <section className="admin-login-panel"><div className="admin-login-box"><div className="admin-login-mobile-logo"><img src="/media/logo-ayrovi.png" alt="AYROVI" style={{width:28,height:28,objectFit:"contain"}} /><strong>AYROVI</strong></div><span className="admin-eyebrow">Espace sécurisé</span><h2>Bienvenue.</h2><p>Connectez-vous avec le compte administrateur configuré sur le serveur.</p>
...
const PricingPage: React.FC<{ canWrite:boolean }> = ({canWrite}) => {
  ...
  const pricingFields=[['rateEUR','Taux EUR'],['rateUSD','Taux USD'],['rateGBP','Taux GBP'],['rateJPY','Taux JPY'],['exchangeBufferPercent','Marge change (%)'],['freightPerKgTND','Fret international (TND/kg)'],['localDeliveryTND','Livraison locale (TND)'],['commissionPercent','Commission AYSONIC (%)'],['minimumCommissionTND','Minimum commission (TND)'],['rpdPercent','Redevance douane (%)'],['rpdMinimumTND','Minimum redevance (TND)'],['expressFeeTND','Supplément Express (TND)']];
  ...
  const calculate=async(e:React.FormEvent<HTMLFormElement>)=>{e.preventDefault();const data=new FormData(e.currentTarget);try{const r=await adminApi<any>('/pricing/preview',{method:'POST',body:JSON.stringify({originalPrice:Number(data.get('price')),currency:data.get('currency'),quantity:Number(data.get('quantity')),express:data.get('express')==='on',title:String(data.get('title')||'')})});setPreview(r.data);}catch(error:any){setToast({message:error.message,tone:'error'});}};
  return <><PageHeader title="Prix & taux" description={`Moteur CIF AYSONIC · configuration v${form.version}. Les commandes existantes gardent leur snapshot.`} action={canWrite?<Button busy={busy} onClick={save}>Enregistrer les taux</Button>:undefined}/>
    ...
    <section className="admin-card admin-price-preview"><CardTitle title="Simulateur" subtitle="Résultat CIF calculé par le backend"/><Form onSubmit={calculate}><Field label="Prix source"><input name="price" type="number" min="0.01" step="0.01" defaultValue="50" required/></Field><Field label="Devise"><Select name="currency" defaultValue="EUR" options={options(['EUR','USD','GBP','JPY','TND'])}/></Field><Field label="Quantité"><input name="quantity" type="number" min="1" defaultValue="1"/></Field><Field label="Titre / mots-clés"><input name="title" type="text" defaultValue="sneakers Nike"/></Field><Field label="Express"><input name="express" type="checkbox"/></Field><Button type="submit">Calculer</Button></Form>{preview&&<div className="admin-price-result"><div><span>Converti</span><b>{formatMoney(preview.convertedPriceTND)}</b></div><div><span>Fret</span><b>{formatMoney(preview.freightTND)}</b></div><div><span>CIF</span><b>{formatMoney(preview.cifTND)}</b></div><div><span>Droit+TVA+RPD</span><b>{formatMoney(preview.customsFeeTND)}</b></div><div><span>Commission</span><b>{formatMoney(preview.serviceFeeTND)}</b></div><div><span>Local</span><b>{formatMoney(preview.localDeliveryTND)}</b></div><strong><span>Total</span><b>{formatMoney(preview.totalTND)}</b></strong></div>}</section></div>{toast&&<Toast {...toast}/>}</>;
};
...
const DesignPage:React.FC<{canWrite:boolean}>=({canWrite})=>{
  ...
  return <><PageHeader title="Développement & design" description="Thème AYROVI (violet, jaune et blanc), canaux sociaux et texte du pied de page. La police Plus Jakarta Sans est unifiée avec AYROVIX." action={canWrite?<Button busy={busy} onClick={save}>Publier le design</Button>:undefined}/>
  <div className="admin-report-grid">
