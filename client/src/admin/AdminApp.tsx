...
const LoginPage: React.FC<{ onAuthenticated: (user: UserIdentity) => void }> = ({ onAuthenticated }) => {
  ...
  return (
    <main className="admin-login">
      <section className="admin-login-brand"><div className="admin-login-brand__mark"><img src="/media/logo-ayrovi.png" alt="AYSONIC" style={{width:40,height:40,objectFit:"contain"}} /></div><span>AYSONIC / CONTROL</span><h1>Le commerce mondial,<br />piloté depuis Tunis.</h1><p>Contenu, arrivages, commandes, tarification et assistance — une seule source de vérité.</p><div className="admin-login-grid"><article><strong>24</strong><span>gouvernorats</span></article><article><strong>4</strong><span>rôles sécurisés</span></article><article><strong>100%</strong><span>backend-driven</span></article></div></section>
      <section className="admin-login-panel"><div className="admin-login-box"><div className="admin-login-mobile-logo"><img src="/media/logo-ayrovi.png" alt="AYSONIC" style={{width:28,height:28,objectFit:"contain"}} /><strong>AYSONIC</strong></div><span className="admin-eyebrow">Espace sécurisé</span><h2>Bienvenue.</h2><p>Connectez-vous avec le compte administrateur configuré sur le serveur.</p>
...
const DashboardPage: React.FC = () => {
  ...
  return <>
    <PageHeader title="Tableau de bord" description="L’activité AYSONIC consolidée en temps réel depuis SQLite." action={<Select value={days} onChange={(event) => setDays(Number(event.target.value))} options={[{value:'7',label:'7 jours'},{value:'30',label:'30 jours'},{value:'90',label:'90 jours'},{value:'365',label:'12 mois'}]} />} />
...
const PageHeader: React.FC<{ title: string; description: string; action?: React.ReactNode }> = ({ title, description, action }) => <div className="admin-page-header"><div><span className="admin-eyebrow">AYSONIC ADMIN</span><h1>{title}</h1><p>{description}</p></div>{action}</div>;
...
const AssistantSupportPage: React.FC<{ canWrite: boolean; requestedId?: string }> = ({ canWrite, requestedId }) => {
  ...
  return <>
    <PageHeader title="Support IA" description="Demandes transférées par Claude à l’équipe AYSONIC, avec identité vérifiée ou contact visiteur." />
    <section className="admin-list-card">
