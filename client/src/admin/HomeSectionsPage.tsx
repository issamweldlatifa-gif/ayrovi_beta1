            ...
            {rows.map((row, index) => (
              <li key={row.id} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, flexWrap: 'wrap' }}>
                ...
                <span style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                  ...
                  <Button variant="ghost" disabled={!canWrite || index === 0} onClick={() => move(index, -1)} aria-label="Monter" title={moveHint(-1, !canWrite, index === 0)}><ArrowUp size={15} /></Button>
                  <Button variant="ghost" disabled={!canWrite || index === rows.length - 1} onClick={() => move(index, 1)} aria-label="Descendre" title={moveHint(1, !canWrite, index === rows.length - 1)}><ArrowDown size={15} /></Button>
                </span>
              </li>
            ))}
          </ul>
