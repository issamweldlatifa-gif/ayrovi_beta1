...
import {
  ...
  LensBox, Menu, Monitor, MousePointer2, Navigation, Palette, RotateCcw, Save,
  ShoppingBag, SlidersHorizontal, Sparkles, Type, User,
} from '../components/QatafoIcons';
import {
  DEFAULT_INTERFACE_CONFIG, INTERFACE_FONT_PRESETS, normalizeInterfaceConfig,
  type InterfaceFontPresetId, type InterfaceSectionConfig, type PublicInterfaceConfig,
...
const NAV_ICONS: React.ElementType[] = [LensBox, Sparkles, AyroviEye];
...
const IconLibraryPreview: React.FC<{ config: PublicInterfaceConfig['icons']; compact?: boolean }> = ({ config, compact }) => {
  ...
  return <div className={`interface-icon-preview ${compact ? 'is-compact' : ''}`} style={{ color: config.color }}>
    {icons.map((Icon, index) => <Icon key={index} size={config.size} style={{ color: index === 1 ? config.activeColor : config.color }} fill={config.style === 'solid' ? 'currentColor' : undefined} />)}
  </div>;
};
...
export const InterfaceStudio: React.FC<{ canWrite: boolean }> = ({ canWrite }) => {
  ...
  return <>
    <div className="admin-page-header">
      <div><span className="admin-eyebrow">AYROVI ADMIN · FULL INTERFACE CONTROL</span><h1>واجهتي</h1><p>Studio des contenus et de la mise en page. La typographie officielle A est verrouillée : Zalando Sans et Noto Sans Arabic.</p></div>
    ...
    <section className="admin-card" role="note">
      <strong>Identité client : AYROVI A · v3</strong>
      <p>La palette blanc pur/noir pur, les deux polices locales et les formes carrées sont fixées par la charte validée. Les réglages de couleurs et de polices ci-dessous restent des réglages historiques, pas un aperçu de la boutique actuelle. Le logo, les contenus, les médias, l’ordre, la visibilité, la mise en page et la navigation restent pilotés ici.</p>
    </section>
    <div className="interface-studio-layout">
      ...
      <div className="interface-studio-content">
          ...
          <section className="admin-card interface-card"><header><div><LayoutGrid size={19} /><div><h2>Contrôle complet des sections</h2><p>Ordre, visibilité, textes, média, fond, texte, espacement et largeur de chaque bloc.</p></div></div></header>
            <div className="interface-section-list">{orderedSections.map((section, index) => <article key={section.id} className={!section.visible ? 'is-hidden' : ''}>
              ...
              <details className="interface-section-advanced"><summary><Palette size={14} />Style propre à cette section</summary><div>
                ...
                <Field label="Largeur du bloc"><Switch disabled={!canWrite} checked={section.contained} onLabel="Contenu encadré" offLabel="Pleine largeur" onChange={() => patchSection(section.id, { contained: !section.contained })} /></Field>
              </div></details>
            </article>)}</div>
          </section>
        </>}
        ...
        {panel === 'typography' && <>
          <section className="admin-card interface-card"><header><div><Type size={19} /><div><h2>Identité officielle AYROVI A</h2><p>Zalando Sans + Noto Sans Arabic : les deux seules familles autorisées. Les contenus et la mise en page restent modifiables.</p></div></div></header><div className="interface-font-presets">{INTERFACE_FONT_PRESETS.map((preset) => <button key={preset.id} type="button" disabled aria-pressed={config.typography.preset === preset.id} className={config.typography.preset === preset.id ? 'is-active' : ''} onClick={() => applyFontPreset(preset.id)} style={{ fontFamily: preset.body }}><span style={{ fontFamily: preset.display }}>Aa أب</span><strong>{preset.label}</strong><small>{preset.description}</small></button>)}</div></section>
          <section className="admin-card interface-card"><header><div><Type size={19} /><div><h2>Réglages typographiques détaillés</h2><p>Polices, couleurs, échelle, rythme, alignement et espacement.</p></div></div></header><div className="admin-form">
            <Field label="Police des titres"><Select disabled value={config.typography.display} onChange={(event) => patch('typography', { display: event.target.value })} options={INTERFACE_FONT_PRESETS.map((preset) => ({ value: preset.display, label: preset.label }))} /></Field>
            <Field label="Police du contenu"><Select disabled value={config.typography.body} onChange={(event) => patch('typography', { body: event.target.value })} options={INTERFACE_FONT_PRESETS.map((preset) => ({ value: preset.body, label: preset.label }))} /></Field>
            ...
            <Field label={`Espacement lettres · ${config.typography.letterSpacing.toFixed(3)}em — verrouillé`}><input disabled type="range" min="-0.08" max="0.12" step="0.005" value={config.typography.letterSpacing} onChange={(event) => patch('typography', { letterSpacing: Number(event.target.value) })} /></Field>
