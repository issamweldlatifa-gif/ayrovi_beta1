    ...
    <div className="space-y-2.5" dir={direction}>
      {visible.map((candidate, index) => {
        ...
        return (
          ...
          >
            {index === 0 && <p className="mb-2 text-xs font-extrabold uppercase tracking-[0.1em] text-ink">{tr('Meilleure correspondance', 'أفضل تطابق')}</p>}
            <div className="flex gap-3">
              <div className="relative grid h-[92px] w-[74px] flex-none place-items-center overflow-hidden rounded-xl border border-line bg-surface p-1.5">
                ...
                <span className={`absolute left-1 top-1 rounded-full px-1.5 py-0.5 text-xs font-extrabold ${candidate.match >= 80 ? 'bg-ink text-white' : 'border border-line bg-white/90 text-ink'}`}>{candidate.match}%</span>
              </div>
              <div className="min-w-0 flex-1">
                ...
                <div className="mt-1.5 flex flex-wrap items-center gap-x-3 gap-y-1">
                  ...
                  <p className="inline-flex items-center gap-1 text-xs font-bold text-ink" title={merchantRating ? tr('Note publiée par le marchand', 'تقييم منشور لدى المتجر') : tr('Score de pertinence converti sur 5', 'درجة التطابق محوّلة إلى 5')}><Star size={13} fill="currentColor" />{rating.toFixed(1)}/5 <span className="text-muted">{merchantRating ? tr('marchand', 'المتجر') : tr('pertinence', 'التطابق')}</span>{merchantRating && Number(candidate.ratingCount) > 0 ? <span className="text-muted">({Number(candidate.ratingCount).toLocaleString(locale === 'ar' ? 'ar-TN' : 'fr-FR')})</span> : null}</p>
                </div>
