            ...
            <div className="flex gap-3">
              ...
              <div className="min-w-0 flex-1">
                ...
                <div className="mt-1.5 flex flex-wrap items-center gap-x-3 gap-y-1">
                  ...
                  <p className="inline-flex items-center gap-1 text-[10px] font-bold text-ink" title={merchantRating ? tr('Note publiée par le marchand', 'تقييم منشور لدى المتجر') : tr('Score de pertinence converti sur 5', 'درجة التطابق محوّلة إلى 5')}><Star size={13} fill="currentColor" />{rating.toFixed(1)}/5 <span className="text-muted">{merchantRating ? tr('marchand', 'المتجر') : tr('pertinence', 'التطابق')}</span>{merchantRating && Number(candidate.ratingCount) > 0 ? <span className="text-muted">({Number(candidate.ratingCount).toLocaleString(locale === 'ar' ? 'ar-TN' : 'fr-FR')})</span> : null}</p>
                </div>
