    ...
    <div className="space-y-5" dir={direction}>
      ...
      <section className="border border-line bg-surface-base p-5 shadow-card sm:p-7 lg:p-8">
        ...
        <div className="mt-6 space-y-6">
          ...
          <fieldset>
            ...
            <div className="grid grid-cols-2 gap-2 sm:grid-cols-3">
              <button type="button" onClick={() => setSizeChoice('')} className={`min-h-12 border px-3 text-sm font-black transition ${sizeChoice === '' ? 'border-ink bg-ink text-surface-base' : 'border-line bg-surface-base text-ink hover:border-ink'}`}>{tr('Sans préférence', 'دون تفضيل')}</button>
              {sizeOptions.map((item) => <button type="button" key={item} onClick={() => setSizeChoice(item)} className={`min-h-12 border px-3 text-sm font-black transition ${sizeChoice === item ? 'border-ink bg-ink text-surface-base' : 'border-line bg-surface-base text-ink hover:border-ink'}`}>{item}</button>)}
              <button type="button" onClick={() => setSizeChoice('__other__')} className={`min-h-12 border px-3 text-sm font-black transition ${sizeChoice === '__other__' ? 'border-ink bg-ink text-surface-base' : 'border-line bg-surface-base text-ink hover:border-ink'}`}>{tr('Autre taille', 'مقاس آخر')}</button>
            </div>
        ...
        <div className="mt-6 border-t border-line pt-5">
          <button type="button" onClick={() => { setSubmitted(true); if (canOrder) onOrder({ size: requestedSize, color: color.trim(), option: selectedOption, quantity, customerNote: customerNote.trim(), manualUrl: manualUrl.trim() }); }} disabled={ordering || Number(selectedPrice) <= 0 || selectedCurrency == null} className="flex min-h-14 w-full items-center justify-center gap-2 bg-ink px-6 text-base font-black text-surface-base transition hover:opacity-90 disabled:cursor-not-allowed disabled:opacity-45">
