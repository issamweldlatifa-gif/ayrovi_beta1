...
<div className="flex items-center justify-between text-xs"><span className="font-bold text-slate-500">Acompte ({Number(orderDetail.deposit_percent || 20)}%)</span><strong>{money(orderDetail.deposit_amount_tnd)}</strong></div>
...
{['PENDING','REJECTED'].includes(orderDetail.deposit_status) && orderDetail.payment_method !== 'CARD' && <div className="space-y-2"><input type="file" accept="image/jpeg,image/png,image/webp,application/pdf" onChange={(e) => setProofFile(e.target.files?.[0] || null)} className="block w-full text-xs text-slate-500 file:mr-3 file:border-0 file:bg-[#673de6] file:px-3 file:py-2 file:text-xs file:font-black file:text-white" /><button disabled={!proofFile || proofBusy} onClick={uploadDepositProof} className="flex w-full items-center justify-center gap-2 bg-[#673de6] px-4 py-2.5 text-xs font-black text-white disabled:opacity-50">{proofBusy && <Loader2 className="h-3.5 w-3.5 animate-spin" />}Envoyer la preuve d’acompte (capture / reçu)</button></div>}
...
{orderDetail.tracking_code && <div className="flex items-center justify-between border border-dashed border-[#673de6] bg-[#f5f2ff] p-3"><span className="text-[10px] font-black uppercase tracking-widest text-[#673de6]">Code de suivi</span><strong className="font-mono text-sm text-[#4e28b9]">{orderDetail.tracking_code}</strong></div>}
