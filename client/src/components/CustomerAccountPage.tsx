...
function Status({ value }: { value: string }) {
  ...
  return <span className={`inline-flex px-2.5 py-1 text-[10px] font-black uppercase tracking-wide ${complete ? 'bg-brand/10 text-brand-dark' : cancelled ? 'bg-danger/5 text-danger' : 'bg-surface text-ink border border-line'}`}>{(isArabic ? statusLabelsAr[value] : statusLabels[value]) || value.replaceAll('_', ' ')}</span>;
}
...
export const CustomerAccountPage: React.FC<CustomerAccountPageProps> = ({
      ...
      <div className="mx-auto flex w-full max-w-[460px] flex-1 flex-col px-6 pb-5 pt-6">
        {otpOpen && challengeId ? (
          ...
          <div className="flex flex-1 flex-col">
            ...
            {/* خيار الهاتف */}
            <button type="button" onClick={() => navigation.pushLayer({ id: 'account:phone-login' })} className="mt-5 flex w-full items-center justify-center gap-2 border-t border-line pt-4 text-xs font-black text-brand transition hover:text-muted">
  ...
  <div className="relative shrink-0">
    ...
    <button type="button" onClick={() => avatarFileRef.current?.click()} aria-label={tr('Changer la photo', 'تغيير الصورة')} className="absolute -bottom-1 -end-1 grid h-7 w-7 place-items-center rounded-full bg-brand text-white shadow"><Camera className="h-4 w-4" /></button>
  </div>
  <div className="min-w-0 flex-1">
    ...
    <div className="mt-2 flex gap-2">
      <button type="button" onClick={() => chooseGender('female')} className={`rounded-full px-3 py-1 text-[11px] font-black transition ${selectedGender === 'female' ? 'bg-pink-500 text-white' : 'border border-line text-muted hover:border-brand'}`}>{tr('Femme', 'أنثى')}</button>
      <button type="button" onClick={() => chooseGender('male')} className={`rounded-full px-3 py-1 text-[11px] font-black transition ${selectedGender === 'male' ? 'bg-sky-500 text-white' : 'border border-line text-muted hover:border-brand'}`}>{tr('Homme', 'ذكر')}</button>
    </div>
  </div>
</div>
<input ref={avatarFileRef} type="file" accept="image/*" className="hidden" onChange={handleAvatarUpload} />
  ...
  const renderAppearance = () => <div className="mx-auto max-w-xl rounded-3xl border border-line bg-white p-6"><Moon className="h-9 w-9 text-brand"/><h3 className="mt-4 text-xl font-black">{tr('Mode sombre','الوضع الداكن')}</h3><p className="mt-2 text-sm leading-6 text-muted">{tr('Cette préférence est enregistrée dans votre compte et reste après reconnexion.','يُحفظ هذا الخيار في حسابك ويبقى بعد تسجيل الدخول مجددًا.')}</p><button onClick={()=>savePreferences({...rows,dark_mode:accountDark?0:1})} className="ay-btn-primary mt-5 w-full text-sm">{accountDark?tr('Passer au mode clair','الانتقال للوضع الفاتح'):tr('Activer le mode sombre','تفعيل الوضع الداكن')}</button></div>;

  const renderSection = () => {
    if(loading)return <div className="grid min-h-64 place-items-center"><Loader2 className="h-8 w-8 animate-spin text-brand"/></div>;
    ...
    if(section==='lensHelp')return <div className="mx-auto max-w-2xl rounded-3xl border border-line bg-white p-6"><ScanSearch className="h-9 w-9 text-brand"/><h3 className="mt-4 text-xl font-black">{tr('Aide AYROVIX Lens','مساعدة عدسة AYROVIX')}</h3><p className="mt-3 text-sm leading-7 text-muted">{tr('Photographiez le produit entier, gardez le prix et la variante visibles, puis vérifiez le lien source avant de l’ajouter au panier.','صوّر المنتج كاملًا وأبقِ السعر والخصائص ظاهرة ثم تحقق من رابط المصدر قبل إضافته للسلة.')}</p></div>;
  ...
  return <div className="ayrovix-theme-scope fixed inset-0 z-[95] overflow-hidden bg-surface" dir={direction} role="dialog" aria-modal="true" aria-label={session?tr('Mon compte AYROVI','حسابي في AYROVI'):tr('Connexion client AYROVI','تسجيل الدخول إلى AYROVI')}>
    ...
    {orderLayer&&orderDetail&&<div className="fixed inset-0 z-[110] overflow-y-auto bg-surface"><AppHeader sticky title={orderDetail.order_number} subtitle={tr('Détail de la commande','تفاصيل الطلب')} onBack={closeAccountLayer}/><main className="mx-auto max-w-5xl space-y-5 px-4 py-5 sm:px-7">
      <section className="rounded-3xl bg-brand-gradient p-5 text-white"><div className="flex flex-wrap justify-between gap-4"><div><p className="text-xs text-white/65">{date(orderDetail.created_at,true)}</p><h2 className="mt-1 text-2xl font-black">{orderDetail.order_number}</h2><div className="mt-3"><Status value={orderDetail.status}/></div></div><strong className="text-2xl font-black text-white">{money(orderDetail.total_tnd)}</strong></div></section>
      {(()=>{const map:Record<string,number>={CREATED:0,AWAITING_DEPOSIT:1,AWAITING_PAYMENT_VERIFICATION:2,CONFIRMED:3,PREPARING:4,SHIPPED:5,IN_TRANSIT:5,OUT_FOR_DELIVERY:5,DELIVERED:6};const stage=map[orderDetail.status]??0;const steps=[tr('Acompte','العربون'),tr('Validation','التحقق'),tr('Confirmée','مؤكد'),tr('Préparation','التجهيز'),tr('Expédition','الشحن'),tr('Livraison','التسليم')];if(orderDetail.status==='CANCELLED')return <div className="rounded-2xl border border-danger/20 bg-danger/5 p-4 text-sm font-black text-danger">{tr('Commande annulée.','تم إلغاء الطلب.')}</div>;return <ol className="grid grid-cols-3 gap-2 rounded-2xl border border-line bg-white p-4 sm:grid-cols-6">{steps.map((label,index)=>{const n=index+1;const done=stage>n;const current=stage===n;return <li key={label} className="min-w-0 text-center"><span className={`mx-auto grid h-9 w-9 place-items-center rounded-full text-xs font-black ${done?'bg-brand text-white':current?'border-2 border-brand text-brand':'bg-surface text-muted'}`}>{done?<Check className="h-7 w-7"/>:n}</span><small className="mt-1 block break-words text-[9px] font-black text-muted">{label}</small></li>})}</ol>})()}
