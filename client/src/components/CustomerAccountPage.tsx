...
export const CustomerAccountPage: React.FC<CustomerAccountPageProps> = ({
  ...
  const selectDepositMethod = async (method: 'CARD' | 'BANK_TRANSFER') => {
  ...
  const renderAddresses = () => <div className="space-y-4"><button onClick={()=>editAddress()} className="ay-btn-primary text-sm"><Plus className="h-4 w-4"/>{tr('Ajouter une adresse','إضافة عنوان')}</button>{Array.isArray(rows)&&rows.length?<div className="grid gap-4 lg:grid-cols-2">{rows.map((address:CustomerAddress)=><article key={address.id} className="min-w-0 rounded-2xl border border-line bg-white p-5"><div className="flex justify-between gap-3"><div className="min-w-0"><h3 className="font-black">{address.label}{Boolean(address.is_default)&&<span className="ms-2 rounded-full bg-brand/10 px-2 py-1 text-[9px] text-brand">{tr('Par défaut','الافتراضي')}</span>}</h3><p className="mt-3 text-sm font-bold">{address.recipient_name}</p><p className="mt-1 break-words text-sm leading-6 text-muted">{address.address_line}<br/>{address.city?`${address.city}, `:''}{address.governorate}{address.postal_code?` ${address.postal_code}`:''}<br/>{address.phone}</p></div><MapPin className="h-5 w-5 shrink-0 text-brand"/></div><div className="mt-4 flex gap-2 border-t border-line pt-3"><button onClick={()=>editAddress(address)} className="px-3 py-2 text-xs font-black text-brand"><Pencil className="me-1 inline h-3.5 w-3.5"/>{tr('Modifier','تعديل')}</button><button onClick={()=>deleteAddress(address.id)} className="px-3 py-2 text-xs font-black text-danger"><Trash2 className="me-1 inline h-3.5 w-3.5"/>{tr('Supprimer','حذف')}</button></div></article>)}</div>:<Empty icon={MapPin} title={tr('Aucune adresse enregistrée','لا يوجد عنوان محفوظ')} text={tr('Ajoutez une adresse de livraison réelle.','أضف عنوان تسليم حقيقيًا.')}/>}</div>;

  const renderOrders = () => Array.isArray(rows)&&rows.length?<div className="space-y-3">{rows.map((order: CustomerOrderSummary)=><button key={order.id} onClick={()=>openOrder(order.id)} className="grid w-full min-w-0 grid-cols-[52px_minmax(0,1fr)_auto] items-center gap-3 rounded-2xl border border-line bg-white p-3 text-start sm:p-4"><div className="grid h-13 w-13 place-items-center overflow-hidden bg-brand/10">{order.image_url?<img src={order.image_url} alt="" className="h-full w-full object-contain"/>:<Package className="h-5 w-5 text-brand"/>}</div><div className="min-w-0"><strong className="block truncate text-sm">{order.order_number}</strong><span className="block text-xs text-muted">{date(order.created_at)} · {order.item_count} {tr('article(s)','منتج')}</span><span className="mt-2 block"><Status value={order.status}/></span></div><div className="text-end"><strong className="block whitespace-nowrap text-sm">{money(order.total_tnd)}</strong><ArrowRight className={`ms-auto mt-2 h-4 w-4 text-muted ${isArabic?'rotate-180':''}`}/></div></button>)}</div>:<Empty icon={Package} title={tr('Aucune commande','لا توجد طلبات')} text={tr('Une commande apparaît ici immédiatement après sa création.','يظهر الطلب هنا فور إنشائه.')}/>;

  const renderFavorites = () => Array.isArray(rows)&&rows.length?<div className="grid grid-cols-2 gap-3 lg:grid-cols-3">{rows.map((favorite: CustomerFavorite)=><article key={favorite.id} className="min-w-0 overflow-hidden rounded-2xl border border-line bg-white"><a href={favorite.source_url||'#'} target={favorite.source_url?'_blank':undefined} rel="noreferrer" className="block aspect-square bg-surface">{favorite.image_url?<img src={favorite.image_url} alt={favorite.title} className="h-full w-full object-contain"/>:<span className="grid h-full place-items-center"><Heart className="h-7 w-7 text-brand"/></span>}</a><div className="p-3"><h3 className="line-clamp-2 text-sm font-black">{favorite.title}</h3>{favorite.price_tnd!=null&&<strong className="mt-2 block text-sm text-brand">{money(favorite.price_tnd)}</strong>}<button onClick={()=>removeFavorite(favorite.id)} className="mt-3 text-[11px] font-black text-danger">{tr('Retirer','إزالة')}</button></div></article>)}</div>:<Empty icon={Heart} title={tr('Aucun favori','لا توجد مفضلة')} text={tr('Vos favoris réels seront conservés ici.','ستُحفظ مفضلاتك الحقيقية هنا.')}/>;

  const renderCart = () => Array.isArray(rows)&&rows.length?<div className="space-y-3">{rows.map((item:CartItem)=><article key={item.id} className="flex min-w-0 gap-3 rounded-2xl border border-line bg-white p-4"><div className="h-16 w-16 shrink-0 bg-surface">{item.imageUrl?<img src={item.imageUrl} alt="" className="h-full w-full object-contain"/>:<ShoppingBag className="m-5 h-6 w-6 text-brand"/>}</div><div className="min-w-0 flex-1"><h3 className="truncate text-sm font-black">{item.title}</h3><p className="text-xs text-muted">{item.variant||item.store}</p><strong className="text-sm text-brand">{money(item.lineTotalTND??item.priceTND)}</strong></div><div className="flex shrink-0 flex-col items-end justify-between"><button onClick={()=>updateCart(item,0)}><Trash2 className="h-4 w-4 text-danger"/></button><div className="flex border border-line"><button onClick={()=>updateCart(item,Math.max(0,item.quantity-1))} className="h-8 w-8">−</button><span className="grid min-w-7 place-items-center text-xs font-black">{item.quantity}</span><button onClick={()=>updateCart(item,item.quantity+1)} className="h-8 w-8">+</button></div></div></article>)}<button onClick={onOpenCart} className="ay-btn-primary w-full text-sm">{tr('Ouvrir le panier','فتح السلة')}</button></div>:<Empty icon={ShoppingBag} title={tr('Votre panier est vide','سلّتك فارغة')} text={tr('Les articles ajoutés apparaîtront ici.','ستظهر المنتجات المضافة هنا.')}/>;

  const renderNotifications = () => <div className="space-y-3">{Array.isArray(rows)&&rows.length&&unreadCount>0?<button onClick={markNotificationsRead} className="ay-btn-secondary text-xs"><Check className="h-4 w-4"/>{tr('Tout marquer comme lu','تحديد الكل كمقروء')}</button>:null}{Array.isArray(rows)&&rows.length?rows.map((item: CustomerNotification)=><article key={item.id} className={`flex gap-3 rounded-2xl border p-4 ${item.read_at?'border-line bg-white':'border-brand/30 bg-brand/5'}`}><span className="grid h-10 w-10 shrink-0 place-items-center rounded-xl bg-brand text-white">{item.type==='ORDER'?<Package className="h-4 w-4"/>:<Bell className="h-4 w-4"/>}</span><div className="min-w-0"><h3 className="text-sm font-black">{item.title}</h3><p className="mt-1 break-words text-sm leading-5 text-muted">{item.message}</p><time className="text-[10px] text-muted">{date(item.created_at,true)}</time></div></article>):<Empty icon={Bell} title={tr('Aucune notification','لا توجد إشعارات')} text={tr('Les événements de commande, paiement, facture et livraison apparaîtront ici.','ستظهر أحداث الطلب والدفع والفاتورة والتسليم هنا.')}/>}</div>;
      ...
      <div className="grid gap-5 lg:grid-cols-[minmax(0,1.15fr)_minmax(0,.85fr)]"><div className="space-y-5">
        ...
        <section className="rounded-2xl border border-line bg-white p-5"><div className="flex items-center justify-between gap-2"><h3 className="font-black">{tr('Paiement de l’acompte','دفع العربون')}</h3><Status value={orderDetail.payment_status}/></div><p className="mt-2 text-xs text-muted">{tr('Méthode','الطريقة')} : {orderDetail.payment_method==='PENDING_SELECTION'?tr('À choisir','اختر الطريقة'):orderDetail.payment_method}</p>
          {orderDetail.status==='AWAITING_DEPOSIT'&&orderDetail.payment_status!=='PAID'&&<div className="mt-4 grid gap-3">
            ...
            <article className={`rounded-2xl border p-4 ${['BANK_TRANSFER','POSTE'].includes(orderDetail.payment_method)?'border-brand bg-brand/5':'border-line'}`}><ReceiptText className="h-5 w-5 text-brand"/><strong className="mt-2 block text-sm">{tr('Virement bancaire / postal','تحويل بنكي / بريدي')}</strong><p className="mt-1 text-[11px] leading-5 text-muted">{tr('Le justificatif sera vérifié par AYROVI; son envoi ne confirme pas le paiement.','تتحقق AYROVI من الإثبات ورفعه لا يعني تأكيد الدفع.')}</p>{!['BANK_TRANSFER','POSTE'].includes(orderDetail.payment_method)&&<button onClick={()=>selectDepositMethod('BANK_TRANSFER')} disabled={paymentBusy} className="ay-btn-secondary mt-3 w-full text-xs">{tr('Choisir le virement','اختيار التحويل')}</button>}</article>
          </div>}
          {['BANK_TRANSFER','POSTE'].includes(orderDetail.payment_method)&&orderDetail.status==='AWAITING_DEPOSIT'&&<div className="mt-4 space-y-3 rounded-2xl border border-brand/20 bg-brand/5 p-4 text-xs"><p><strong>{orderDetail.paymentOptions?.transfer?.companyName}</strong></p>{orderDetail.paymentOptions?.transfer?.bankRib&&<p className="break-all">RIB : <strong>{orderDetail.paymentOptions.transfer.bankRib}</strong></p>}{orderDetail.paymentOptions?.transfer?.posteAccount&&<p className="break-all">{tr('Compte postal','الحساب البريدي')} : <strong>{orderDetail.paymentOptions.transfer.posteAccount}</strong></p>}{!orderDetail.paymentOptions?.transfer?.bankRib&&!orderDetail.paymentOptions?.transfer?.posteAccount&&<p className="font-bold text-warning">{tr('Coordonnées de virement non publiées par l’administration.','لم تنشر الإدارة بيانات التحويل بعد.')}</p>}<Field label={tr('Référence du virement / versement','مرجع التحويل / الإيداع')}><input className={inputClass} value={transferReference} onChange={(e)=>setTransferReference(e.target.value.slice(0,120))} placeholder="Référence réelle"/></Field><input type="file" accept="image/jpeg,image/png,application/pdf" onChange={(e)=>setProofFile(e.target.files?.[0]||null)} className="block w-full min-w-0 text-xs"/><button onClick={uploadDepositProof} disabled={!proofFile||!transferReference.trim()||proofBusy} className="ay-btn-primary w-full text-xs">{proofBusy&&<Loader2 className="h-4 w-4 animate-spin"/>}{tr('Envoyer le justificatif','إرسال الإثبات')}</button></div>}
