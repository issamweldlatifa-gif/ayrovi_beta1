...
function Status({ value }: { value: string }) {
  ...
  return <span className={`inline-flex px-2.5 py-1 text-[10px] font-black uppercase tracking-wide ${complete ? 'bg-surface text-ink' : cancelled ? 'bg-danger/5 text-danger' : 'bg-surface text-ink border border-line'}`}>{(isArabic ? statusLabelsAr[value] : statusLabels[value]) || value.replaceAll('_', ' ')}</span>;
}
...
const inputClass = 'min-h-12 w-full rounded-xl border border-line bg-white px-3.5 py-3 text-sm font-semibold text-ink outline-none transition focus:border-line focus:ring-2 focus:ring-black/10/10 disabled:bg-surface disabled:text-muted';
...
export const CustomerAccountPage: React.FC<CustomerAccountPageProps> = ({
      ...
      <div className="mx-auto flex w-full max-w-[460px] flex-1 flex-col px-6 pb-5 pt-6">
        {otpOpen && challengeId ? (
          ...
          <form onSubmit={verifyCode} className="flex flex-1 flex-col">
            ...
            <div className="mx-auto grid h-16 w-16 place-items-center rounded-3xl bg-surface text-ink"><Phone className="h-8 w-8" aria-hidden /></div>
            ...
            {developmentCode && <button type="button" onClick={() => setCode(developmentCode)} className="mt-3 w-full border border-line bg-surface p-3 text-xs font-bold text-ink">{tr('Mode développement — utiliser', 'وضع التطوير — استخدم')} {developmentCode}</button>}
          ...
          <form onSubmit={sendCode} className="flex flex-1 flex-col">
            ...
            {config?.phoneOtp.enabled === false && <p className="mt-3 text-center text-xs font-semibold text-muted">{tr('L’envoi SMS doit être configuré sur le serveur.', 'يجب ضبط خدمة SMS على الخادم.')}</p>}
            <p className="mt-auto pt-8 text-center text-[11px] leading-5 text-muted">{tr('Connexion sécurisée. AYROVI ne vous demandera jamais votre code par téléphone.', 'دخول آمن. لن تطلب منك AYROVI رمزك عبر مكالمة أو رسالة.')}</p>
          </form>
        ) : (
          /* ===== الشاشة الرئيسية ===== */
  ...
  const renderHome = () => {
    ...
    return <div className="space-y-5">
      <section className="relative overflow-hidden rounded-3xl bg-ink-gradient p-5 text-white sm:p-7"><div className="relative flex items-center gap-4"><AccountAvatar account={session!.account} size="h-11 w-11" uploadedUrl={uploadedAvatar} gender={selectedGender} /><div className="min-w-0"><p className="text-[10px] font-black uppercase tracking-[0.18em] text-white/70">{tr('Mon compte', 'حسابي')}</p><h2 className="mt-1 truncate text-2xl font-black">{session!.account.displayName || tr('Client AYROVI', 'حريف AYROVI')}</h2><p className="truncate text-xs text-white/70">{session!.account.email || session!.account.phone}</p></div></div></section>
  ...
  <div className="min-w-0 flex-1">
    ...
    <div className="mt-2 flex gap-2">
      <button type="button" onClick={() => chooseGender('female')} className={`rounded-full px-3 py-1 text-[11px] font-black transition ${selectedGender === 'female' ? 'bg-pink-500 text-white' : 'border border-line text-muted hover:border-line'}`}>{tr('Femme', 'أنثى')}</button>
      <button type="button" onClick={() => chooseGender('male')} className={`rounded-full px-3 py-1 text-[11px] font-black transition ${selectedGender === 'male' ? 'bg-sky-500 text-white' : 'border border-line text-muted hover:border-line'}`}>{tr('Homme', 'ذكر')}</button>
    </div>
  </div>
</div>
<input ref={avatarFileRef} type="file" accept="image/*" className="hidden" onChange={handleAvatarUpload} />
  ...
  const renderAddresses = () => <div className="space-y-4"><button onClick={()=>editAddress()} className="ay-btn-primary text-sm">{tr('Ajouter une adresse','إضافة عنوان')}</button>{Array.isArray(rows)&&rows.length?<div className="grid gap-4 lg:grid-cols-2">{rows.map((address:CustomerAddress)=><article key={address.id} className="min-w-0 rounded-2xl border border-line bg-white p-5"><div className="flex justify-between gap-3"><div className="min-w-0"><h3 className="font-black">{address.label}{Boolean(address.is_default)&&<span className="ms-2 rounded-full bg-surface px-2 py-1 text-[9px] text-ink">{tr('Par défaut','الافتراضي')}</span>}</h3><p className="mt-3 text-sm font-bold">{address.recipient_name}</p><p className="mt-1 break-words text-sm leading-6 text-muted">{address.address_line}<br/>{address.city?`${address.city}, `:''}{address.governorate}{address.postal_code?` ${address.postal_code}`:''}<br/>{address.phone}</p></div></div><div className="mt-4 flex gap-2 border-t border-line pt-3"><button onClick={()=>editAddress(address)} className="px-3 py-2 text-xs font-black text-ink">{tr('Modifier','تعديل')}</button><button onClick={()=>deleteAddress(address.id)} className="px-3 py-2 text-xs font-black text-danger">{tr('Supprimer','حذف')}</button></div></article>)}</div>:<Empty title={tr('Aucune adresse enregistrée','لا يوجد عنوان محفوظ')} text={tr('Ajoutez une adresse de livraison réelle.','أضف عنوان تسليم حقيقيًا.')}/>}</div>;
  ...
  const renderAppearance = () => <div className="mx-auto max-w-xl rounded-3xl border border-line bg-white p-6"><Moon className="h-9 w-9 text-ink"/><h3 className="mt-4 text-xl font-black">{tr('Mode sombre','الوضع الداكن')}</h3><p className="mt-2 text-sm leading-6 text-muted">{tr('Cette préférence est enregistrée dans votre compte et reste après reconnexion.','يُحفظ هذا الخيار في حسابك ويبقى بعد تسجيل الدخول مجددًا.')}</p><button onClick={()=>savePreferences({...rows,dark_mode:accountDark?0:1})} className="ay-btn-primary mt-5 w-full text-sm">{accountDark?tr('Passer au mode clair','الانتقال للوضع الفاتح'):tr('Activer le mode sombre','تفعيل الوضع الداكن')}</button></div>;
  ...
  const renderSection = () => {
    ...
    if(section==='lensHelp')return <div className="mx-auto max-w-2xl rounded-3xl border border-line bg-white p-6"><ScanSearch className="h-9 w-9 text-ink"/><h3 className="mt-4 text-xl font-black">{tr('Aide AYROVIX Lens','مساعدة عدسة AYROVIX')}</h3><p className="mt-3 text-sm leading-7 text-muted">{tr('Photographiez le produit entier, gardez le prix et la variante visibles, puis vérifiez le lien source avant de l’ajouter au panier.','صوّر المنتج كاملًا وأبقِ السعر والخصائص ظاهرة ثم تحقق من رابط المصدر قبل إضافته للسلة.')}</p></div>;
  ...
  return <div className="ayrovix-theme-scope fixed inset-0 z-[95] overflow-hidden bg-surface" dir={direction} role="dialog" aria-modal="true" aria-label={session?tr('Mon compte AYROVI','حسابي في AYROVI'):tr('Connexion client AYROVI','تسجيل الدخول إلى AYROVI')}>
    ...
    {orderLayer&&orderDetail&&<div className="fixed inset-0 z-[110] overflow-y-auto bg-surface"><AppHeader sticky title={orderDetail.order_number} subtitle={tr('Détail de la commande','تفاصيل الطلب')} onBack={closeAccountLayer}/><main className="mx-auto max-w-5xl space-y-5 px-4 py-5 sm:px-7">
      <section className="rounded-3xl bg-ink-gradient p-5 text-white"><div className="flex flex-wrap justify-between gap-4"><div><p className="text-xs text-white/65">{date(orderDetail.created_at,true)}</p><h2 className="mt-1 text-2xl font-black">{orderDetail.order_number}</h2><div className="mt-3"><Status value={orderDetail.status}/></div></div><strong className="text-2xl font-black text-white">{money(orderDetail.total_tnd)}</strong></div></section>
      {(()=>{const map:Record<string,number>={CREATED:0,AWAITING_DEPOSIT:1,AWAITING_PAYMENT_VERIFICATION:2,CONFIRMED:3,PREPARING:4,SHIPPED:5,IN_TRANSIT:5,OUT_FOR_DELIVERY:5,DELIVERED:6};const stage=map[orderDetail.status]??0;const steps=[tr('Acompte','العربون'),tr('Validation','التحقق'),tr('Confirmée','مؤكد'),tr('Préparation','التجهيز'),tr('Expédition','الشحن'),tr('Livraison','التسليم')];if(orderDetail.status==='CANCELLED')return <div className="rounded-2xl border border-danger/20 bg-danger/5 p-4 text-sm font-black text-danger">{tr('Commande annulée.','تم إلغاء الطلب.')}</div>;return <ol className="grid grid-cols-3 gap-2 rounded-2xl border border-line bg-white p-4 sm:grid-cols-6">{steps.map((label,index)=>{const n=index+1;const done=stage>n;const current=stage===n;return <li key={label} className="min-w-0 text-center"><span className={`mx-auto grid h-9 w-9 place-items-center rounded-full text-xs font-black ${done?'bg-ink text-white':current?'border-2 border-line text-ink':'bg-surface text-muted'}`}>{done?<Check className="h-7 w-7"/>:n}</span><small className="mt-1 block break-words text-[9px] font-black text-muted">{label}</small></li>})}</ol>})()}
      <div className="grid gap-5 lg:grid-cols-[minmax(0,1.15fr)_minmax(0,.85fr)]"><div className="space-y-5">
        ...
        <section className="rounded-2xl border border-line bg-white p-5"><h3 className="font-black">{tr('Historique vérifié','السجل الموثق')}</h3><div className="mt-4 space-y-4">{orderDetail.history.map((item: CustomerOrderHistoryEntry)=><div key={item.id} className="flex gap-3"><span className="mt-1 h-3 w-3 shrink-0 rounded-full bg-ink"/><div className="min-w-0"><Status value={item.to_status}/><p className="mt-1 break-words text-xs text-muted">{item.note}</p><time className="text-[10px] text-muted">{date(item.created_at,true)}</time></div></div>)}</div></section>
        ...
        <section className="rounded-2xl border border-line bg-white p-5"><div className="flex items-center justify-between gap-2"><h3 className="font-black">{tr('Paiement de l’acompte','دفع العربون')}</h3><Status value={orderDetail.payment_status}/></div><p className="mt-2 text-xs text-muted">{tr('Méthode','الطريقة')} : {orderDetail.payment_method==='PENDING_SELECTION'?tr('À choisir','اختر الطريقة'):orderDetail.payment_method}</p>
          ...
          {['BANK_TRANSFER','POSTE'].includes(orderDetail.payment_method)&&orderDetail.status==='AWAITING_DEPOSIT'&&(()=>{
            ...
            return <div className="mt-4 space-y-3 rounded-2xl border border-line bg-surface p-4 text-xs">
              ...
              {coordinates?<p className="break-all">{isPostal?tr('Compte postal','الحساب البريدي'):'RIB'} : <strong>{coordinates}</strong></p>
                :<p className="font-bold text-muted">{tr('Coordonnées officielles non publiées; l’envoi du justificatif reste bloqué.','البيانات الرسمية غير منشورة؛ إرسال الإثبات معطل.')}</p>}
              {coordinates&&<>
                ...
                <button onClick={uploadDepositProof} disabled={!proofFile||!transferReference.trim()||proofBusy} className="ay-btn-primary w-full text-xs">{proofBusy&&<Loader2 className="h-7 w-7 animate-spin"/>}{tr('Envoyer le justificatif','إرسال الإثبات')}</button>
              </>}
            </div>;
          })()}
