      ...
      <div className={`ay-auth__container ${otpOpen || phoneLoginOpen || phoneLinkOpen ? 'ay-auth__container--utility' : ''}`}>
        {otpOpen && challengeId ? (
          ...
          <form onSubmit={verifyCode} className="flex flex-1 flex-col">
            ...
            <div className="mx-auto grid h-16 w-16 place-items-center rounded-card bg-surface text-ink"><Phone className="h-8 w-8" aria-hidden /></div>
            <h1 className="mt-5 text-center text-3xl font-black leading-9 tracking-[-0.02em] text-ink">{tr('Entrez votre code', 'أدخل رمز التحقق')}</h1>
          ...
          <form onSubmit={sendCode} className="flex flex-1 flex-col">
            ...
            <div className="mx-auto grid h-16 w-16 place-items-center rounded-card bg-surface text-ink"><Phone className="h-8 w-8" aria-hidden /></div>
            <h1 className="mt-5 text-center text-3xl font-black leading-9 tracking-[-0.02em] text-ink">{phoneLinkOpen ? tr('Vérifier mon téléphone', 'توثيق هاتفي') : tr('Continuer par téléphone', 'المتابعة عبر الهاتف')}</h1>
      ...
      <div className="grid gap-5 lg:grid-cols-[minmax(0,1.15fr)_minmax(0,.85fr)]"><div className="space-y-5">
        ...
        <section className="rounded-2xl border border-line bg-white p-5"><div className="flex items-center justify-between gap-2"><h3 className="font-black">{tr('Paiement de l’acompte','دفع العربون')}</h3><Status value={orderDetail.payment_status}/></div><p className="mt-2 text-xs text-muted">{tr('Méthode','الطريقة')} : {orderDetail.payment_method==='PENDING_SELECTION'?tr('À choisir','اختر الطريقة'):orderDetail.payment_method}</p>
          {orderDetail.status==='AWAITING_DEPOSIT'&&orderDetail.payment_status!=='PAID'&&<div className="mt-4 grid gap-3">
            ...
            <article className={`rounded-2xl border p-4 ${['BANK_TRANSFER','POSTE'].includes(orderDetail.payment_method)?'border-line bg-surface':'border-line'}`}><ReceiptText className="h-7 w-7 text-ink"/><strong className="mt-2 block text-sm">{tr('Virement bancaire / postal','تحويل بنكي / بريدي')}</strong><p className="mt-1 text-xs leading-5 text-muted">{tr('Le justificatif sera vérifié par AYROVI; son envoi ne confirme pas le paiement.','تتحقق AYROVI من الإثبات ورفعه لا يعني تأكيد الدفع.')}</p>{!orderDetail.paymentOptions.transfer.bankRib&&!orderDetail.paymentOptions.transfer.posteAccount&&<p className="mt-2 text-xs font-bold text-muted">{tr('Indisponible jusqu’à la publication des coordonnées officielles.','غير متاح حتى نشر البيانات الرسمية.')}</p>}{!['BANK_TRANSFER','POSTE'].includes(orderDetail.payment_method)&&<button onClick={()=>selectDepositMethod(orderDetail.paymentOptions.transfer.bankRib?'BANK_TRANSFER':'POSTE')} disabled={paymentBusy||(!orderDetail.paymentOptions.transfer.bankRib&&!orderDetail.paymentOptions.transfer.posteAccount)} className="ay-btn-secondary mt-3 w-full text-xs">{tr('Choisir le virement','اختيار التحويل')}</button>}</article>
          </div>}
          ...
          {orderDetail.transactions?.length>0&&<div className="mt-4 space-y-2 border-t border-line pt-4">{orderDetail.transactions.map((tx: CustomerPaymentTransaction)=><div key={tx.id} className="flex min-w-0 justify-between gap-3 text-xs"><span className="min-w-0 break-all ay-number">{tx.transaction_number}</span><Status value={tx.status}/></div>)}</div>}
        </section>
        <section className="rounded-2xl border border-line bg-white p-5"><h3 className="font-black">{tr('Facture & suivi','الفاتورة والتتبع')}</h3>{orderDetail.invoice?<a href={`/api/customer/account/orders/${orderDetail.id}/invoice`} className="ay-btn-secondary mt-4 w-full text-xs"><ArrowDown className="h-7 w-7"/>{orderDetail.invoice.invoice_number}</a>:<p className="mt-3 text-xs leading-5 text-muted">{tr('Aucune facture émise pour le moment.','لم تصدر فاتورة حتى الآن.')}</p>}{orderDetail.delivery?.tracking_number?<div className="mt-4 rounded-2xl border border-line bg-surface p-4 text-xs"><strong className="block">{orderDetail.delivery.carrier}</strong><span className="mt-1 block break-all ay-number">{orderDetail.delivery.tracking_number}</span>{safeAccountLink(orderDetail.delivery.tracking_url)&&<a href={safeAccountLink(orderDetail.delivery.tracking_url)} target="_blank" rel="noreferrer" className="mt-3 inline-flex items-center gap-1 font-black text-ink"><ExternalLink className="h-3.5 w-3.5"/>{tr('Ouvrir le suivi','فتح التتبع')}</a>}</div>:<p className="mt-3 text-xs leading-5 text-muted">{tr('Le suivi apparaîtra après l’expédition réelle.','سيظهر التتبع بعد الشحن الفعلي.')}</p>}</section>
