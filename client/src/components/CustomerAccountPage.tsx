...
export const CustomerAccountPage: React.FC<CustomerAccountPageProps> = ({
  ...
  const renderHome = () => {
    ...
    return <div className="space-y-5">
      <section className="relative overflow-hidden rounded-3xl bg-brand-gradient p-5 text-white sm:p-7"><div className="relative flex items-center gap-4"><AccountAvatar account={session!.account} size="h-11 w-11" uploadedUrl={uploadedAvatar} gender={selectedGender} /><div className="min-w-0"><p className="text-[10px] font-black uppercase tracking-[0.18em] text-accent">{tr('Mon compte', 'حسابي')}</p><h2 className="mt-1 truncate text-2xl font-black">{session!.account.displayName || tr('Client AYROVI', 'حريف AYROVI')}</h2><p className="truncate text-xs text-white/70">{session!.account.email || session!.account.phone}</p></div></div></section>
