...
import {
  ...
  MapPin, Package, Pencil, Phone, Plus, ShieldCheck, ShoppingBag, Trash2, User, X, Hourglass, AlertCircle, ArrowDown, Truck,
} from './QatafoIcons';
...
function Status({ value }: { value: string }) {
  ...
  return <span className={`inline-flex px-2.5 py-1 text-[10px] font-black uppercase tracking-wide ${complete ? 'bg-brand/10 text-brand-dark' : cancelled ? 'bg-danger/5 text-danger' : 'bg-brand/15 text-white'}`}>{(isArabic ? statusLabelsAr[value] : statusLabels[value]) || value.replaceAll('_', ' ')}</span>;
}
...
export const CustomerAccountPage: React.FC<CustomerAccountPageProps> = ({
  ...
  const trackingLayer = [...navigation.stack].reverse().find((layer) => layer.id === 'account:tracking');
  ...
  const openTracking = (id: string) => {
    if (!id) return;
    navigation.pushLayer({ id: 'account:tracking', payload: { orderId: id } });
  };
  ...
  const authPanel = (
    <div className="mx-auto flex min-h-full w-full max-w-lg flex-col justify-center px-5 py-10 sm:my-10 sm:min-h-0 sm:rounded-card sm:border sm:border-line sm:bg-white sm:px-10 sm:py-12 sm:shadow-overlay">
      <div className="mb-8 text-center"><img src="/media/logo-ayrovi-final.png?v=fig3" alt="AYROVI" className="mx-auto h-20 w-20 object-contain" /><h1 className="mt-5 text-3xl font-black tracking-[-0.045em] text-ink">{phoneLinkOpen ? tr('Vérifier mon téléphone', 'توثيق هاتفي') : tr('Bienvenue chez AYROVI', 'مرحبًا بك في AYROVI')}</h1><p className="mt-2 text-sm leading-6 text-muted">{phoneLinkOpen ? (session?.account.emailVerified ? tr('Ajoutez un second canal vérifié pour renforcer votre compte.', 'أضف قناة موثقة ثانية لتعزيز أمان حسابك.') : tr('Un e-mail ou un téléphone vérifié est requis pour confirmer une commande.', 'يلزم بريد إلكتروني أو هاتف موثّق لتأكيد الطلب.')) : tr('Votre panier, vos commandes et vos adresses sur tous vos appareils.', 'سلّتك وطلباتك وعناوينك متاحة على جميع أجهزتك.')}</p></div>
  ...
  const renderHome = () => {
    ...
    return <div className="space-y-6">
      <section className="relative overflow-hidden bg-brand-gradient p-6 text-white sm:p-8"><div className="absolute -right-10 -top-12 h-40 w-40 rounded-full bg-brand/70 blur-2xl" /><p className="relative text-xs font-black uppercase tracking-[0.18em] text-white/70">{tr('Mon espace AYROVI', 'فضائي في AYROVI')}</p><h2 className="relative mt-2 text-3xl font-black tracking-tight">{tr('Bonjour', 'مرحبًا')}، {session!.account.displayName || tr('Client AYROVI', 'حريف AYROVI')}</h2><p className="relative mt-2 text-sm text-white/65">{tr(`${formatMoney(overview?.totalSpent)} commandés au total`, `إجمالي الطلبات ${formatMoney(overview?.totalSpent)}`)}</p></section>
  ...
  const renderOrders = () => rows.length ? <div className="space-y-3">{rows.map((order) => <article key={order.id} className="border border-line bg-surface-base shadow-card transition hover:border-brand"><button type="button" onClick={() => void openOrder(order.id)} className="grid w-full grid-cols-[56px_minmax(0,1fr)_auto] items-center gap-3 p-4 text-start"><div className="grid h-14 w-14 place-items-center overflow-hidden bg-brand/10 text-brand">{order.image_url ? <img src={order.image_url} alt="" className="h-full w-full object-cover" /> : <Package className="h-5 w-5" />}</div><div className="min-w-0"><strong className="block truncate text-sm text-ink">{order.order_number}</strong><span className="mt-1 block text-xs text-muted">{formatDate(order.created_at)} · {order.item_count} {tr('article(s)', 'منتج')}</span><span className="mt-2 block"><Status value={order.status} /></span></div><div className="text-end"><strong className="block text-sm text-ink">{formatMoney(order.total_tnd)}</strong>{busyId === order.id ? <Loader2 className="ms-auto mt-2 h-4 w-4 animate-spin text-brand" /> : <ArrowRight className={`ms-auto mt-2 h-4 w-4 text-muted ${direction === 'rtl' ? 'rotate-180' : ''}`} />}</div></button><div className="flex flex-wrap items-center justify-between gap-3 border-t border-line px-4 py-3"><div className="min-w-0"><span className="block text-[9px] font-black uppercase tracking-[0.14em] text-muted">{tr('Suivi', 'التتبع')}</span><strong className="block truncate font-mono text-xs text-ink">{order.tracking_code || tr('En préparation', 'قيد الإعداد')}</strong></div><button type="button" onClick={() => openTracking(order.id)} className="flex min-h-10 items-center gap-2 border border-interactive-primary bg-transparent px-4 text-xs font-black text-interactive-primary transition hover:bg-interactive-primary/5"><Truck className="h-4 w-4" />{tr('Suivre ma commande', 'تتبع طلبي')}</button></div></article>)}</div> : <Empty icon={Package} title={tr('Aucune commande', 'لا توجد طلبات')} text={tr('Après votre premier achat, le suivi détaillé apparaîtra ici.', 'بعد أول عملية شراء سيظهر التتبع المفصل هنا.')} />;
  ...
  return <div className="fixed inset-0 z-[95] overflow-hidden bg-surface" dir={direction} role="dialog" aria-modal="true" aria-label={session ? tr('Mon compte AYROVI', 'حسابي في AYROVI') : tr('Connexion client AYROVI', 'تسجيل الدخول إلى AYROVI')}>
    ...
    {orderLayer && orderDetail && <div className="fixed inset-0 z-[110] overflow-y-auto bg-surface"><AppHeader sticky title={orderDetail.order_number} subtitle={tr('Détail de la commande', 'تفاصيل الطلب')} onBack={closeAccountLayer} /><main className="mx-auto max-w-4xl space-y-5 px-4 py-6 sm:px-7"><section className="bg-brand-gradient p-6 text-white"><div className="flex flex-wrap items-start justify-between gap-4"><div><p className="text-xs font-bold text-white/55">{tr('Commande du', 'طلب بتاريخ')} {formatDate(orderDetail.created_at)}</p><h2 className="mt-2 text-2xl font-black">{orderDetail.order_number}</h2><div className="mt-3"><Status value={orderDetail.status} /></div></div><strong className="text-2xl font-black text-white">{formatMoney(orderDetail.total_tnd)}</strong></div></section>{(() => {
...
{orderDetail.tracking_code && <button type="button" onClick={() => openTracking(orderDetail.id)} className="flex min-h-14 w-full items-center justify-between gap-3 border border-brand bg-brand/5 p-3 text-start transition hover:bg-brand/10"><span><span className="block text-[10px] font-black uppercase tracking-widest text-brand">{tr('Code de suivi · ouvrir la carte', 'رمز التتبع · فتح الخريطة')}</span><strong className="mt-1 block font-mono text-sm text-brand-dark">{orderDetail.tracking_code}</strong></span><Truck className="h-5 w-5 shrink-0 text-brand" /></button>}
...
<div className="mt-3"><Status value={orderDetail.payment_status} /></div></section></div><section className="border border-line bg-white p-5"><h3 className="mb-4 font-black">{tr('Suivi', 'التتبع')}</h3><div className="space-y-4">{orderDetail.history?.map((item: any) => <div key={item.id} className="flex gap-3"><span className="mt-1 h-3 w-3 shrink-0 rounded-full bg-brand" /><div><Status value={item.to_status} /><p className="mt-1 text-xs text-muted">{item.note}</p><time className="text-[10px] text-muted">{formatDate(item.created_at, true)}</time></div></div>)}</div></section></main></div>}
    {trackingLayer && typeof trackingLayer.payload?.orderId === 'string' && <OrderTrackingView orderId={trackingLayer.payload.orderId} onBack={closeAccountLayer} />}
  </div>;
};
