...
export const CustomerAccountPage: React.FC<CustomerAccountPageProps> = ({
  ...
  const renderProfile = () => (
    <form onSubmit={saveProfile} className="max-w-2xl space-y-5 rounded-[28px] border border-slate-200 bg-white p-5 shadow-sm sm:p-7">
      ...
      <label className="flex items-start gap-3 rounded-2xl border border-slate-200 p-4"><input type="checkbox" checked={profile.marketingOptIn} onChange={(event) => setProfile({ ...profile, marketingOptIn: event.target.checked })} className="mt-0.5 h-5 w-5 accent-brand" /><span><strong className="block text-sm text-ink">مجلتي وعروض AYROVI</strong><small className="text-xs leading-5 text-slate-500">Recevoir les offres et nouveaux arrivages.</small></span></label>
