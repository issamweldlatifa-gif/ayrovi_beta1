...
const inputClass = 'min-h-12 w-full rounded-xl border border-slate-200 bg-white px-3.5 py-3 text-sm font-semibold text-ink outline-none transition focus:border-brand focus:ring-2 focus:ring-brand/10 disabled:bg-slate-50 disabled:text-slate-500';
...
export const CustomerAccountPage: React.FC<CustomerAccountPageProps> = ({
  ...
  const authPanel = (
    <div className="mx-auto flex min-h-full w-full max-w-lg flex-col justify-center px-5 py-10 sm:my-10 sm:min-h-0 sm:rounded-[32px] sm:border sm:border-slate-200 sm:bg-white sm:px-10 sm:py-12 sm:shadow-[0_24px_70px_-35px_rgba(36,16,79,0.38)]">
      <div className="mb-8 text-center"><img src="/media/logo-ayrovi.png" alt="AYROVI" className="mx-auto h-20 w-20 object-contain" /><h1 className="mt-5 text-3xl font-black tracking-[-0.045em] text-ink">{phoneLinkOpen ? 'Vérifier mon téléphone' : 'Bienvenue chez AYROVI'}</h1><p className="mt-2 text-sm leading-6 text-slate-500">{phoneLinkOpen ? 'Facultative — elle lie votre téléphone à votre compte pour retrouver vos commandes.' : 'Votre panier, vos commandes et vos adresses sur tous vos appareils.'}</p></div>
      ...
      {!otpOpen && !phoneLinkOpen && <>
        <a href={googleEnabled ? googleStartHref : undefined} aria-disabled={!googleEnabled} className={`flex min-h-12 w-full items-center justify-center gap-3 rounded-xl border px-4 py-3.5 text-sm font-black transition ${googleEnabled ? 'border-ink bg-ink text-white shadow-lg hover:bg-[#2b2340]' : 'pointer-events-none border-slate-300 bg-white text-ink opacity-50'}`}><span className="grid h-7 w-7 place-items-center rounded-full bg-white"><FcGoogle size={20} aria-hidden /></span>Continuer avec Google</a>
        {facebookEnabled && <a href={facebookStartHref} className="mt-3 flex min-h-12 w-full items-center justify-center gap-3 rounded-xl border border-[#1877f2] bg-[#1877f2] px-4 py-3.5 text-sm font-black text-white shadow-lg transition hover:bg-[#0f69dc]"><span className="grid h-7 w-7 place-items-center rounded-full bg-white text-[#1877f2]"><FaFacebookF size={16} aria-hidden /></span>Continuer avec Facebook</a>}
      ...
      {otpOpen && challengeId && <form onSubmit={verifyCode} className="space-y-4">
        ...
        <Field label="Code à 6 chiffres"><input autoFocus inputMode="numeric" autoComplete="one-time-code" pattern="[0-9]{6}" maxLength={6} value={code} onChange={(e) => setCode(e.target.value.replace(/\D/g, ''))} placeholder="000000" className={`${inputClass} text-center font-mono text-2xl tracking-[0.35em]`} required /></Field>
  ...
  const renderProfile = () => (
    <form onSubmit={saveProfile} className="max-w-2xl space-y-5 rounded-[28px] border border-slate-200 bg-white p-5 shadow-sm sm:p-7">
      <div className="flex items-center gap-4 border-b border-slate-100 pb-5">
        <div className="grid h-16 w-16 place-items-center overflow-hidden rounded-2xl bg-brand text-xl font-black text-white shadow-lg shadow-brand/20">
          {session!.account.avatarUrl ? <img src={session!.account.avatarUrl} alt="Photo de profil" className="h-full w-full object-cover" /> : (session!.account.displayName || 'AY').slice(0, 2).toUpperCase()}
        </div>
        <div><h3 className="font-black text-ink">Identité du compte</h3><p className="mt-1 text-xs leading-5 text-slate-500">Gérez les informations utilisées pour vos commandes et notifications.</p></div>
      </div>
      <Field label="Nom et prénom"><input className={inputClass} value={profile.displayName} onChange={(event) => setProfile({ ...profile, displayName: event.target.value.slice(0, 120) })} autoComplete="name" minLength={2} maxLength={120} required /></Field>
      <Field label="Adresse e-mail"><input type="email" className={inputClass} value={profile.email} onChange={(event) => setProfile({ ...profile, email: event.target.value.slice(0, 180) })} placeholder="vous@exemple.com" autoComplete="email" maxLength={180} inputMode="email" /></Field>
      <Field label="Téléphone (vérification facultative)">
        <div className="flex min-h-12 items-center gap-2 rounded-xl border border-slate-200 bg-slate-50 px-3.5 py-3 text-sm font-bold text-slate-600"><Phone className="h-4 w-4 text-brand" />{session!.account.phone || 'Non renseigné'}{session!.account.phoneVerified && <CheckCircle2 className="ml-auto h-4 w-4 text-emerald-600" />}</div>
        {!session!.account.phoneVerified && <button type="button" onClick={() => navigation.pushLayer({ id: 'account:phone-link' })} className="mt-2 flex min-h-11 w-full items-center justify-center gap-2 rounded-xl border border-dashed border-brand/50 bg-brand/5 px-4 py-2.5 text-xs font-black text-brand-dark hover:bg-brand/10"><ShieldCheck className="h-4 w-4" />Vérifier mon téléphone (facultatif)</button>}
      </Field>
      <label className="flex items-start gap-3 rounded-2xl border border-slate-200 p-4"><input type="checkbox" checked={profile.marketingOptIn} onChange={(event) => setProfile({ ...profile, marketingOptIn: event.target.checked })} className="mt-0.5 h-5 w-5 accent-brand" /><span><strong className="block text-sm text-ink">Actualités et promotions AYROVI</strong><small className="text-xs leading-5 text-slate-500">Recevoir les offres et nouveaux arrivages.</small></span></label>
      <button disabled={busyId === 'profile'} className="flex min-h-12 items-center gap-2 rounded-xl bg-brand px-5 py-3 text-sm font-black text-white shadow-lg shadow-brand/15 transition hover:bg-brand-dark disabled:opacity-50">{busyId === 'profile' && <Loader2 className="h-4 w-4 animate-spin" />}Enregistrer le profil</button>
      <section className="border-t border-red-100 pt-5"><h4 className="text-sm font-black text-red-700">Supprimer mon compte</h4><p className="mt-1 text-xs leading-5 text-slate-500">Supprime le profil, les connexions, sessions, adresses et favoris. Les documents de commande déjà créés restent archivés sans accès au compte.</p><button type="button" disabled={busyId === 'delete-account'} onClick={deleteAccount} className="mt-3 flex min-h-11 items-center gap-2 rounded-xl border border-red-300 px-4 py-2.5 text-xs font-black text-red-700 disabled:opacity-50">{busyId === 'delete-account' ? <Loader2 className="h-4 w-4 animate-spin" /> : <Trash2 className="h-4 w-4" />}Supprimer définitivement le compte</button></section>
    </form>
  );
  ...
  return <div className="fixed inset-0 z-[95] overflow-hidden bg-surface" role="dialog" aria-modal="true" aria-label={session ? 'Mon compte AYROVI' : 'Connexion client AYROVI'}>
    <header className="relative z-20 border-b border-slate-200 bg-white"><div className="h-1 bg-accent" /><div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-3 sm:h-20 sm:px-6"><div className="flex items-center gap-2.5"><img src="/media/logo-ayrovi.png" alt="" className="h-10 w-10 object-contain" /><div><strong className="block text-xl font-black leading-none text-ink">AYROVI</strong><span className="text-[9px] font-black uppercase tracking-[0.16em] text-slate-400">Espace client</span></div></div><button autoFocus onClick={onClose} className="grid h-11 w-11 place-items-center border border-slate-200 bg-white text-ink hover:border-brand" aria-label="Fermer"><X className="h-5 w-5" /></button></div></header>
