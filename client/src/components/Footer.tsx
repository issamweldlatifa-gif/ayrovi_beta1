...
export const Footer: React.FC<FooterProps> = ({ onOpenAccount, onOpenAssistant, logoUrl = '/media/logo-ayrovi-final.png', coverImage, introTitle, introText }) => {
  ...
  return (
    <footer className="mt-16 border-t border-white/10 bg-ink pb-8 pt-12 text-white/70">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="mb-9 space-y-3 text-xs">
          <div className="flex items-center gap-2.5">
            <img src={logoUrl} alt="" className="ayrovi-logo h-11 w-11 rounded-full bg-white p-1.5 object-contain" />
            <span className="text-lg font-extrabold tracking-[-0.04em] text-white">{introTitle || 'AYROVI'}</span>
          </div>
          <p className="max-w-xl leading-relaxed text-white/70">{introText || footerAbout}</p>
        </div>
        ...
        {/* قنواتنا الرسمية */}
        {socials.length > 0 && (
          <div id="nos-canaux" className="mb-10 flex flex-col items-center gap-3 rounded-2xl border border-white/10 bg-white/5 px-4 py-6 text-center backdrop-blur-sm">
            <strong className="text-sm font-black uppercase tracking-[0.18em] text-white">Nos canaux officiels</strong>
        ...
        <nav className="mb-10 grid grid-cols-1 gap-0 border-t border-white/10 sm:grid-cols-2 sm:gap-6 sm:border-t-0 lg:grid-cols-5" aria-label="Pied de page">
          {groups.map((group) => (
            <details key={group.title} className="group border-b border-white/10 sm:border-b-0" open={false}>
              <summary className="flex cursor-pointer list-none items-center justify-between py-4 text-[15px] font-bold text-white sm:cursor-default sm:py-0 sm:pb-3 [&::-webkit-details-marker]:hidden">
                {group.title}
                <span className="text-xl font-light text-white/55 transition group-open:rotate-45 sm:hidden">+</span>
              </summary>
        ...
        <div className="flex flex-col items-center gap-4 rounded-2xl bg-ink px-4 py-6 text-center sm:px-8">
          <div className="flex flex-wrap items-center justify-center gap-2.5">
            ...
            <PayBadge label="Flouci" className="bg-white/10 not-italic text-white" />
            <PayBadge label="D17" className="bg-white/10 not-italic text-white" />
            <PayBadge label="Mandat postal" className="bg-white/10 not-italic text-white" />
          </div>
