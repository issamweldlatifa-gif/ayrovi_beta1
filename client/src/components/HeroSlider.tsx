...
interface HeroSliderProps {
  settings?: PublicInterfaceConfig['slider'];
  title?: string;
  subtitle?: string;
  image?: string;
}

export const HeroSlider: React.FC<HeroSliderProps> = ({ settings, title, subtitle, image }) => {
  ...
  const slides = useMemo<RenderedHeroSlide[]>(() => {
    const base = managedSlides.length ? managedSlides.map((slide, index) => ({
      ...slide,
    ...
    })) : FALLBACK_IMAGES.map((slide) => ({ ...slide, fallback: slide.image }));
    return image ? base.map((slide, index) => index === 0 ? { ...slide, image, fallback: slide.fallback } : slide) : base;
  }, [managedSlides, image]);

  useEffect(() => {
    if (activeIndex >= slides.length) setActiveIndex(0);
    if (settings?.autoplay === false || slides.length < 2) return undefined;
    const intervalId = window.setInterval(() => setActiveIndex((current) => (current + 1) % slides.length), settings?.duration || 5200);
    return () => window.clearInterval(intervalId);
  }, [slides.length, settings?.autoplay, settings?.duration]);
