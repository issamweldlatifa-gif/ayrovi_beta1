...
const ToolPresentations = ({ message, isDark, selectedProduct, productBusyId, isOrdering, onSelectProduct, onProductOrder }: Pick<AssistantMessagesProps, 'selectedProduct' | 'productBusyId' | 'isOrdering' | 'onSelectProduct' | 'onProductOrder'> & { message: AssistantMessage; isDark: boolean }) => {
  ...
  return <div className="mt-3 space-y-3">
    ...
    {message.supportTicket && <article className={`flex items-start gap-3 rounded-2xl border p-4 ${isDark ? 'border-white/10 bg-white/[0.04]' : 'border-accent/20 bg-accent/10'}`}><span className="grid h-10 w-10 shrink-0 place-items-center rounded-xl bg-brand text-white"><MessageSquare size={20}/></span><div><p className={`text-[11px] font-bold uppercase tracking-[.12em] ${isDark ? 'text-white/80' : 'text-ink'}`}>{tr('Support AYROVI', 'دعم AYROVI')}</p><h3 className="mt-0.5 text-sm font-extrabold">{tr('Ticket enregistré', 'تم تسجيل التذكرة')}</h3><p className="mt-1 break-all text-[11px] text-muted">{tr('Référence', 'المرجع')} : {message.supportTicket.id}</p></div></article>}
  </div>;
};
