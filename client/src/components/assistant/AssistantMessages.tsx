...
const ShareAction = ({ message, isDark }: { message: AssistantMessage; isDark: boolean }) => {
  ...
  return <button type="button" onClick={() => void share()} aria-label={tr('Partager', 'مشاركة')} className={`rounded-lg p-1.5 transition ${isDark ? 'text-muted hover:bg-white/5 hover:text-white' : 'text-muted hover:bg-surface hover:text-ink'}`}><Share2 size={26} /></button>;
};
  ...
  return <div className="mt-3 space-y-3">
    {message.orderStatuses?.map((order) => (
      <article key={order.orderId} className={`rounded-2xl border p-4 ${isDark ? 'border-white/10 bg-white/[0.04]' : 'border-line/15 bg-surface'}`}>
        <div className="flex items-start gap-3"><span className="grid h-11 w-11 shrink-0 place-items-center rounded-xl bg-ink text-white"><PackageCheck size={30}/></span><div className="min-w-0 flex-1"><p className="text-[11px] font-bold uppercase tracking-[.12em] text-ink">{tr('Commande', 'طلب')} {order.orderId}</p><h3 className="mt-0.5 text-sm font-extrabold">{order.statusLabel}</h3><p className={`mt-1 text-xs ${isDark ? 'text-muted' : 'text-muted'}`}>{order.carrier && `${order.carrier} · `}{order.trackingCode || tr('Suivi en préparation', 'التتبع قيد الإعداد')}</p></div></div>
    ...
    {message.products?.some(isDisplayableCandidate) ? <div className="grid gap-3 sm:grid-cols-2">
      {message.products.filter(isDisplayableCandidate).map((product) => <article key={product.id} className={`overflow-hidden rounded-2xl border ${isDark ? 'border-white/10 bg-white/[0.04]' : 'border-line bg-white'}`}>
        ...
        <div className="p-3">
          ...
          <div className="mt-2 flex flex-wrap items-center justify-between gap-2"><div><p className={`text-base font-black ${isDark ? 'text-white' : 'text-ink'}`}>{product.priceTnd != null ? `${product.priceTnd.toFixed(2)} DT` : '—'}</p><p className={`text-[11px] font-semibold ${isDark ? 'text-muted' : 'text-muted'}`}>{Number(product.price).toFixed(Number(product.price) % 1 ? 2 : 0)} {product.currency}</p><p className="mt-1 inline-flex items-center gap-1 text-[10px] font-bold text-ink"><Star size={22} fill="currentColor" />{displayRating(product).toFixed(1)}/5 <span className={isDark ? 'text-muted' : 'text-muted'}>{product.ratingKind === 'merchant' ? tr('marchand', 'المتجر') : tr('pertinence', 'التطابق')}</span></p></div></div>
    ...
    {message.supportTicket && <article className={`flex items-start gap-3 rounded-2xl border p-4 ${isDark ? 'border-white/10 bg-white/[0.04]' : 'border-line bg-surface'}`}><span className="grid h-11 w-11 shrink-0 place-items-center rounded-xl bg-surface text-ink"><MessageSquare size={30}/></span><div><p className="text-[11px] font-bold uppercase tracking-[.12em] text-muted">{tr('Support AYROVI', 'دعم AYROVI')}</p><h3 className="mt-0.5 text-sm font-extrabold">{tr('Ticket enregistré', 'تم تسجيل التذكرة')}</h3><p className="mt-1 break-all text-[11px] text-muted">{tr('Référence', 'المرجع')} : {message.supportTicket.id}</p></div></article>}
  </div>;
};
          ...
          <div className="assistant-welcome flex flex-1 flex-col">
            ...
            <div className="flex flex-col items-center text-center">
              ...
              {assistantReady === false && (
                <p className="mt-4 max-w-sm rounded-2xl border border-line bg-surface px-4 py-3 text-[12px] font-semibold leading-5 text-ink">
            ...
            {messages.map((message, index) => {
              ...
              return <div key={message.id} className={`flex items-start ${message.role === 'user' ? 'justify-end' : 'justify-start gap-2.5'}`}>
                ...
                <div className={message.role === 'user' ? 'max-w-[82%]' : hasPresentation ? 'min-w-0 flex-1 max-w-[92%]' : 'min-w-0 max-w-[82%]'}>
                  <div className={`rounded-[22px] px-4 py-3 text-[14px] leading-6 ${message.role === 'user' ? 'rounded-br-md bg-ink text-white shadow-card' : isDark ? 'rounded-bl-md border border-white/10 bg-white/[0.055] text-white' : 'rounded-bl-md border border-line bg-white text-ink shadow-card'}`}>
                  ...
                  {message.role === 'assistant' && !isLastAssistantStreaming && <div className="mt-1.5 flex items-center gap-0.5 px-1"><button type="button" onClick={() => onCopy(message)} aria-label={tr('Copier', 'نسخ')} className={`rounded-lg p-1.5 transition ${isDark ? 'text-muted hover:bg-white/5 hover:text-white' : 'text-muted hover:bg-surface hover:text-ink'}`}>{copiedId === message.id ? <Check size={26}/> : <Copy size={26}/>}</button><button type="button" onClick={() => onRegenerate(message.id)} aria-label={tr('Régénérer', 'إعادة التوليد')} className={`rounded-lg p-1.5 transition ${isDark ? 'text-muted hover:bg-white/5 hover:text-white' : 'text-muted hover:bg-surface hover:text-ink'}`}><RefreshCw size={26}/></button><button type="button" aria-label={tr('Lire', 'استماع')} className={`rounded-lg p-1.5 transition ${isDark ? 'text-muted hover:bg-white/5 hover:text-white' : 'text-muted hover:bg-surface hover:text-ink'}`} onClick={() => window.speechSynthesis?.speak(new SpeechSynthesisUtterance(assistantText))}><Volume2 size={26}/></button><ShareAction message={message} isDark={isDark}/><span className={`mx-1 h-4 w-px ${isDark ? 'bg-white/10' : 'bg-line'}`}/><button type="button" onClick={() => onFeedback(message, 'up')} aria-label={tr('Utile', 'مفيد')} className={`rounded-lg p-1.5 ${feedback[message.id] === 'up' ? 'bg-success/10 text-success' : isDark ? 'text-muted hover:bg-white/5' : 'text-muted hover:bg-surface'}`}><ThumbsUp size={26}/></button><button type="button" onClick={() => onFeedback(message, 'down')} aria-label={tr('Pas utile', 'غير مفيد')} className={`rounded-lg p-1.5 ${feedback[message.id] === 'down' ? 'bg-danger/10 text-danger' : isDark ? 'text-muted hover:bg-white/5' : 'text-muted hover:bg-surface'}`}><ThumbsDown size={26}/></button><button type="button" onClick={() => onOpenComment(message)} className={`ms-1 rounded-lg px-2 py-1 text-[10px] font-bold ${isDark ? 'text-muted hover:bg-white/5' : 'text-muted hover:bg-surface'}`}>{tr('Commenter', 'تعليق')}</button></div>}
                </div>
              </div>;
            })}
