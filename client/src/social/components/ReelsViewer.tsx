        ...
        {items.map((post, index) => (
          <motion.section key={post.id} initial={{ opacity: 0.7, scale: 1.01 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ amount: 0.7 }} transition={{ duration: 0.24 }} className="relative h-full w-full snap-start snap-always overflow-hidden">
            <video
              ...
              poster="/media/logo-ayrovi-final.png?v=fig3"
              muted={muted}
              loop
              playsInline
            ...
            />
            {videoFailed[post.id] && <div className="absolute inset-0 grid place-items-center bg-ink"><img src="/media/logo-ayrovi-final.png?v=fig3" alt="" className="h-24 w-24 rounded-card bg-white object-contain p-3" /></div>}
