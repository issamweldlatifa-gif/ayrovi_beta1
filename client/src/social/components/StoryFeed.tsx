    ...
    <div className="flex items-center gap-2.5 px-4 pb-3 pt-4 sm:px-5">
      ...
      <div className="min-w-0 flex-1 leading-tight">
        <p className={`flex items-center gap-1 text-sm font-extrabold ${light ? 'text-white' : 'text-ink'}`}>
          {post.publisher.name}
          {post.publisher.verified && <CheckCircle2 size={14} className={`shrink-0 ${light ? 'text-white' : 'text-ink'}`} />}
        </p>
      ...
      <div className="relative">
        <button type="button" aria-label={tr('Options du post', 'خيارات المنشور')} onClick={() => setMenuOpen((open) => !open)} className={`grid h-11 w-11 place-items-center rounded-control transition active:scale-90 ${light ? 'text-white hover:bg-white/15' : 'text-muted hover:bg-surface'}`}>
          <MoreVertical size={18} />
        </button>
        {menuOpen && (
          <div className="absolute end-0 top-10 z-10 w-44 overflow-hidden rounded-card border border-line bg-white shadow-lg">
            <button type="button" className="block w-full px-4 py-2.5 text-start text-xs font-bold text-ink hover:bg-surface" onClick={async () => { try { if (!navigator.clipboard?.writeText) throw new Error('CLIPBOARD_UNAVAILABLE'); await navigator.clipboard.writeText(postPublicUrl(post.id)); setCopyStatus('copied'); } catch { setCopyStatus('error'); } }}>{tr('Copier le lien', 'نسخ الرابط')}</button>
            <p className="px-4 py-2.5 text-xs text-muted">{tr('Signalement indisponible', 'الإبلاغ غير متاح')}</p>
            {copyStatus !== 'idle' && <p role="status" className="px-4 pb-3 text-xs text-ink">{copyStatus === 'copied' ? tr('Lien du post copié.', 'نُسخ رابط المنشور.') : tr('Copie impossible. Vérifiez les autorisations.', 'تعذر النسخ. تحقق من الأذونات.')}</p>}
          </div>
        )}
      </div>
    </div>
  ...
  if (post.type === 'video' || post.media[0]?.type === 'video') {
    return (
      <button type="button" onClick={() => onOpenReels?.(post)} aria-label={tr('Ouvrir la vidéo', 'فتح الفيديو')} className="relative block w-full">
        ...
        />
        <span className="absolute bottom-2 end-2 rounded-control bg-black/45 px-2.5 py-1 text-xs font-black uppercase tracking-widest text-white">{tr('Reels', 'ريلز')}</span>
      </button>
    );
  }
...
const PostActions: React.FC<{
  ...
  <div className="flex items-center gap-1 px-3 pt-3">
    <button type="button" onClick={onLike} aria-label={liked ? tr('Ne plus aimer', 'إلغاء الإعجاب') : tr('Aimer', 'إعجاب')} className={`grid h-12 w-12 place-items-center rounded-control transition active:scale-90 ${liked ? 'heart-pop text-cta' : 'text-ink hover:bg-surface'}`}>
      {liked ? <HeartFilled size={24} /> : <Heart size={24} />}
    </button>
    <button type="button" onClick={onComment} aria-label={tr('Commenter', 'تعليق')} className="grid h-12 w-12 place-items-center rounded-control text-ink transition hover:bg-surface active:scale-90">
      <MessageSquare size={23} />
    </button>
    <button type="button" onClick={() => void sharePost(post)} aria-label={tr('Partager', 'مشاركة')} className="grid h-12 w-12 place-items-center rounded-control text-ink transition hover:bg-surface active:scale-90">
      <Share2 size={23} />
    </button>
    <button type="button" onClick={toggleSave} aria-label={tr('Enregistrer', 'حفظ')} className={`ms-auto grid h-12 w-12 place-items-center rounded-control transition active:scale-90 ${saved ? 'text-ink' : 'text-ink hover:bg-surface'}`}>
...
export const StoryPostCard: React.FC<{
  ...
  const toggleLike = async () => {
    ...
    try {
      const result = await likePost(post.id, !liked);
  ...
  return (
    <motion.article initial={{ opacity: 0, y: 14 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, amount: 0.15 }} transition={{ duration: 0.32 }} className="border-b border-line bg-white pb-4">
      {likeError && <p role="alert" className="px-4 py-2 text-xs text-danger">{tr("Confirmation du geste indisponible. Vérifiez la connexion.", "تعذر تأكيد التفاعل. تحقق من الاتصال.")}</p>}
      {isVideo ? (
        <div className="relative">
      ...
      <div className="px-4 sm:px-5">
        ...
        {post.cta && <button type="button" onClick={() => onCta(post.cta!)} className="mt-3 inline-flex min-h-11 items-center gap-2 rounded-control bg-surface px-4 text-xs font-extrabold text-ink transition hover:bg-ink/15 active:scale-95">{post.cta.label}<ArrowRight size={16} className={direction === 'rtl' ? 'rotate-180' : ''} /></button>}
        <button type="button" onClick={() => (isAuthenticated ? onOpenComments(post) : onRequireAuth())} className="mt-3 flex min-h-12 w-full items-center gap-3 rounded-control border border-line bg-surface px-3 text-start text-sm font-semibold text-muted transition focus:border-line hover:border-line/40">
