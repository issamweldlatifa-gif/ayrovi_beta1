...
<style>
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .brand small { display: block; font-size: 11px; color: #6E6E73; font-weight: 600; margin-top: 6px; letter-spacing: 0; }
  ...
  .card { flex: 1; border: 1px solid #D2D2D7; border-radius: 12px; padding: 14px 16px; }
  ...
  <div style="max-width:520px;margin:0 auto;background:#ffffff;border-radius:16px;overflow:hidden;border:1px solid #D2D2D7;">
    ...
    <div style="padding:26px;">
      ...
      <table style="width:100%;margin:18px 0;border-collapse:collapse;font-size:14px;">
        <tr><td style="padding:8px 0;color:#6E6E73;">Facture</td><td style="text-align:right;font-weight:700;">${e(input.invoiceNumber)}</td></tr>
        <tr><td style="padding:8px 0;color:#6E6E73;">Acompte encaissé</td><td style="text-align:right;font-weight:700;color:#1D1D1F;">${e(input.depositLabel)}</td></tr>
        <tr><td style="padding:8px 0;color:#6E6E73;">Solde à la livraison</td><td style="text-align:right;font-weight:700;">${e(input.balanceLabel)}</td></tr>
        ${input.trackingCode ? `<tr><td style="padding:8px 0;color:#6E6E73;">Code de suivi</td><td style="text-align:right;font-weight:700;color:#1D1D1F;font-family:monospace;">${e(input.trackingCode)}</td></tr>` : ''}
      </table>
      <p style="line-height:1.6;color:#1D1D1F;">Votre facture électronique est jointe à cet e-mail en PDF. Vous pouvez aussi la télécharger à tout moment depuis votre espace client AYROVI (Commandes → ${e(input.orderNumber)}).</p>
      <p style="margin-top:22px;font-size:12px;color:#6E6E73;">Merci de votre confiance — l'équipe ${e(input.company)}</p>
    </div>
  </div>
</body></html>`;
}
