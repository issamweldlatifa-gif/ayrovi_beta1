...
const candidate: AyrovixCandidate={id:'jacket',kind:'external',title:'K-Way LIL — Veste mi-saison',brand:'K-Way',model:null,colors:[],sizes:[],source:'Example',sourceUrl:'https://merchant-shop.com/item',image:'/jacket.jpg',price:179.95,currency:'EUR',priceTnd:650,match:94,rating:4.3,ratingCount:27,ratingKind:'merchant'};
...
describe('reference Lens product card',()=>{
 it('shows brand, description, one unmodified TND price, and no redundant commercial copy',()=>{
  const html=render();expect(html).toContain('K-Way');expect(html).toContain('LIL — Veste mi-saison');expect(html).toContain('650,000 DT');expect(html).toContain('Prix marchand');expect(html).toContain('179,95');
 ...
 it.each([null,0,-1,NaN,Infinity])('shows confirmation instead of a fabricated price for %s',priceTnd=>{
  const html=render({priceTnd});expect(html).toContain('Prix à confirmer');expect(html).not.toContain('0,000 DT');
 });
 it('does not substitute the uploaded image for a missing merchant product image',()=>{
  const html=render({image:'',images:[]});expect(html).toContain('ay-studio-frame__placeholder');expect(html).not.toContain('<img');
 });
 ...
 it('preserves SerpApi reviews from both supported rating fields and never uses match as a rating',async()=>{
  ...
  vi.stubGlobal('fetch',vi.fn(async()=>new Response(JSON.stringify({visual_matches:[
   {title:'K-Way LIL jacket',brand:'K-Way',description:'Merchant description',link:'https://merchant-shop.com/a',price:{extracted_value:179.95,currency:'EUR'},rating:4.6,reviews:137},
   {title:'Black jacket',link:'https://merchant-shop.com/b',price:{extracted_value:100,currency:'EUR'},product_rating:4.2,reviews_count:26},
   {title:'No rating jacket',link:'https://merchant-shop.com/c',price:{extracted_value:90,currency:'EUR'},exact_matches:true},
  ]}),{status:200})));
