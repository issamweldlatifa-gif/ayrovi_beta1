...
function Fixture(){const nav=useNavigationHistory();const layer=lens?'app:lens':'app:assistant';return <><button data-open onClick={()=>nav.pushLayer({id:layer})}>Open test surface</button>{nav.has(layer)&&(lens?<LensLauncher isOpen onClose={()=>nav.back()} onOrder={async payload=>{orders.push(payload);}} cartCount={0} onOpenCart={()=>{}} darkMode={false} onToggleDarkMode={()=>{}}/>:<AiAssistantDrawer isOpen onClose={()=>nav.back()} onOpenLens={()=>{}} onOpenOrders={()=>{}} onOpenAccount={()=>{}} onOrder={async payload=>{orders.push(payload);}}/>)}</>;}
