...
function Fixture() {
  ...
  return <><button data-open onClick={() => navigation.pushLayer({ id: 'app:assistant' })}>SONIM test fixture</button>
    {navigation.has('app:assistant') && <AiAssistantDrawer isOpen historyScope={historyScope} onClose={() => navigation.back()} onOpenLens={() => {}} onOpenOrders={() => {}} onOpenAccount={() => {}} onOpenCart={() => {}} onOrder={async payload => { orders.push(payload); }}/>}
  </>;
}
