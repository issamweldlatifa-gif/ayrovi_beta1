...
try{
 for(const locale of ['fr','ar'])for(const width of [320,390]){
  ...
  const choose=async()=>{await page.evaluate(()=>document.querySelectorAll('details').forEach(details=>{details.open=true;}));await page.getByRole('combobox',{name:ar?'المقاس':'Taille',exact:true}).selectOption('M');await page.locator('input[list]').fill('Bleu');};
  ...
  await page.evaluate(()=>document.querySelectorAll('details').forEach(details=>{details.open=true;}));const quantity=page.getByRole('spinbutton',{name:ar?'الكمية':'Quantité',exact:true});await quantity.fill('99');
